...
export default async function handler(req, res) {
  ...
  try {
    switch (action) {
      case 'bonus_hunt':       return await handleBonusHunt(res, userId);
      case 'bonus_hunt_history': return await handleBonusHuntHistory(res, userId, req.query);
      case 'overlay_state':   return await handleOverlayState(res, userId);
      case 'widgets':         return await handleWidgets(res, userId);
      case 'profile':         return await handleProfile(res, userId);
      default:
    ...
    console.error('[streamer-data]', err);
...
async function handleBonusHunt(res, userId) {
  ...
  const bestBonus = openedBonuses.reduce((best, b) => {
    const m = b.bet && b.result ? parseFloat(b.result) / parseFloat(b.bet) : 0;
    const bm = best.bet && best.result ? parseFloat(best.result) / parseFloat(best.bet) : 0;
    return m > bm ? b : best;
  }, openedBonuses[0] || {});

  const bestMulti = bestBonus?.bet && bestBonus?.result
    ? parseFloat(bestBonus.result) / parseFloat(bestBonus.bet) : 0;
  ...
  return res.json({
    hunts: (data || []).map(h => {
      ...
      const bestBonus = bonuses.reduce((best, b) => {
        const m = b.bet && b.result ? b.result / b.bet : 0;
        const bm = best.bet && best.result ? best.result / best.bet : 0;
        return m > bm ? b : best;
      }, bonuses[0] || {});
      return {
        id: h.id,
...
async function handleOverlayState(res, userId) {
  const { data } = await supabase
    ...
    .eq('user_id', userId)
    .single();
  return res.json({ state: data?.state || {}, updated_at: data?.updated_at || null });
}
...
async function handleWidgets(res, userId) {
  ...
  const safeWidgets = (data || []).map(w => {
    ...
    return { type: w.widget_type, label: w.label, visible: w.is_visible, config, updated_at: w.updated_at };
  });
...
async function handleProfile(res, userId) {
  const { data: profile } = await supabase
    .from('user_profiles')
    .select('twitch_username, twitch_display_name, avatar_url')
    .eq('user_id', userId)
    .single();

  return res.json({
    username: profile?.twitch_display_name || profile?.twitch_username || 'Streamer',

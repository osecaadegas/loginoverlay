...
export default async function handler(req, res) {
  ...
  // ── Route: Scrape slot info from external sites (?action=scrape) ──
  const action = (req.query?.action || '').toLowerCase();
  if (action === 'scrape') {
    return handleScrapeSlotInfo(req, res);
  }
...
// ═══════════════════════════════════════════════════════════════════════
// SCRAPE HANDLER (?action=scrape) — Merged from fetch-slot-info.js
// GET /api/slot-ai?action=scrape&name=Hot+Ross
// ═══════════════════════════════════════════════════════════════════════

const SCRAPE_UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36';

function scrapeDecodeHtml(html) {
  return html
...
function scrapeStripTags(html) {
  return scrapeDecodeHtml(html.replace(/<[^>]+>/g, '')).trim();
}

async function scrapeFetch(url) {
  try {
    const r = await fetch(url, { headers: { 'User-Agent': SCRAPE_UA, 'Accept': 'text/html' }, redirect: 'follow' });
    return r.ok ? r.text() : null;
  } catch { return null; }
}

function scrapeMapVolatility(v) {
  if (!v) return null;
...
function parseDemoSlot(html, slug) {
  ...
  if (!info.provider) { const p = html.match(/Provider\s*<\/[^>]+>\s*<[^>]+>\s*<a[^>]*>(.*?)<\/a>/i); if (p) info.provider = scrapeStripTags(p[1]); }
  if (!info.rtp) { const r = html.match(/RTP\s*<\/[^>]+>\s*<[^>]+>\s*([\d.]+)%/i); if (r) info.rtp = parseFloat(r[1]); }
  if (!info.volatility) { const v = html.match(/Volatility\s*<\/[^>]+>\s*<[^>]+>\s*(\w[\w ]*\w|\w)/i); if (v) info.volatility = v[1].trim().toLowerCase(); }
  if (!info.max_win_multiplier) { const x = html.match(/Max Win\s*<\/[^>]+>\s*<[^>]+>\s*([\d,]+)x/i); if (x) info.max_win_multiplier = parseFloat(x[1].replace(/,/g, '')); }
  ...
  if (wpPostMatch) { info.image = wpPostMatch[1]; }
  else { const slugImg = allImgs.find(u => u.toLowerCase().includes(slug)); info.image = slugImg || allImgs.find(u => !/mostbet|casinia|casino|bonus|banner|logo|welcome|ad-/i.test(u.split('/').pop())) || allImgs[0] || null; }
  return info;
}
...
function parseSlotArk(html, slug) {
  ...
  if (provMatch) info.provider = scrapeStripTags(provMatch[1]).replace(/\b\w+/g, w => w[0].toUpperCase() + w.slice(1).toLowerCase());
...
async function fetchDemoSlotPage(slug, name, fast) {
  ...
  for (const url of patterns) { const html = await scrapeFetch(url); if (html) return { html, url }; }
  if (fast) return null;
  const searchHtml = await scrapeFetch(`https://www.bing.com/search?q=site:demoslot.com+${encodeURIComponent(name)}+slot`);
  if (searchHtml) { const link = searchHtml.match(/href="(https:\/\/www\.demoslot\.com\/free-slots\/[^"]+)"/); if (link) { const html = await scrapeFetch(link[1]); if (html) return { html, url: link[1] }; } }
  return null;
}

async function fetchSlotArkPage(slug, name, fast) {
  const url = `https://www.slotark.com/slots/${slug}/`;
  const html = await scrapeFetch(url);
  if (html) return { html, url };
  if (fast) return null;
  const searchHtml = await scrapeFetch(`https://www.bing.com/search?q=site:slotark.com+${encodeURIComponent(name)}+slot`);
  if (searchHtml) { const link = searchHtml.match(/href="(https:\/\/www\.slotark\.com\/slots\/[^"]+)"/); if (link) { const page = await scrapeFetch(link[1]); if (page) return { html: page, url: link[1] }; } }
  return null;
}

async function fetchSlotsLaunchPage(slug, name, fast) {
  if (fast) return null;
  const searchHtml = await scrapeFetch(`https://www.bing.com/search?q=site:slotslaunch.com+${encodeURIComponent(name)}+slot`);
  if (searchHtml) { const link = searchHtml.match(/href="(https:\/\/slotslaunch\.com\/[^"]+\/[^"]+)"/); if (link) { const html = await scrapeFetch(link[1]); if (html) return { html, url: link[1] }; } }
  return null;
}

async function handleScrapeSlotInfo(req, res) {
  ...
  try {
    ...
    const [demoResult, arkResult, launchResult] = await Promise.all([
      srcParam.includes('demoslot') ? fetchDemoSlotPage(slug, name, fast) : null,
      srcParam.includes('slotark') ? fetchSlotArkPage(slug, name, fast) : null,
      srcParam.includes('slotslaunch') ? fetchSlotsLaunchPage(slug, name, fast) : null,
    ]);
    const demoInfo = demoResult ? parseDemoSlot(demoResult.html, slug) : {};
    const arkInfo = arkResult ? parseSlotArk(arkResult.html, slug) : {};
    const launchInfo = launchResult ? parseSlotsLaunch(launchResult.html) : {};
    if (!demoResult && !arkResult && !launchResult) return res.status(200).json({ ok: true, info: null, notFound: true });
    const info = {
      provider: demoInfo.provider || arkInfo.provider || launchInfo.provider || null,
      rtp: demoInfo.rtp || arkInfo.rtp || null,
      volatility: scrapeMapVolatility(demoInfo.volatility || arkInfo.volatility),
    ...
    console.error('[slot-ai/scrape] Error:', err);

 ...
 * Each theme has: id, name, category, colors (primary, secondary, accent, background, text, panelBg, border), font
 * Categories: core, seasonal, glass, luxury, glossy, matte, anodized, metallic, vibrant, vivid, fx
 */

const THEMES = {
  // ── Core Themes ──
  'default': { colors: { primary: '#00e1ff', secondary: '#9147ff', accent: '#a855f7', background: 'rgba(26, 27, 46, 0.95)', text: '#f3f4f6', panelBg: 'linear-gradient(135deg, rgba(26, 27, 46, 0.95) 0%, rgba(22, 33, 62, 0.95) 100%)', border: 'rgba(255, 255, 255, 0.1)' }, font: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" },
...
export const THEME_CATEGORIES = [
  ...
  { id: 'anodized', label: 'Anodized', emoji: '🔧' },
  { id: 'metallic', label: 'Metallic', emoji: '⚙️' },
  { id: 'vibrant', label: 'Vibrant', emoji: '🌈' },
  { id: 'vivid', label: 'Vivid', emoji: '💎' },
  { id: 'fx', label: 'FX Animated', emoji: '🔥' },
];

/** Map theme id → category */
function getCategory(id) {
  if (id === 'default') return 'core';
  if (['cyberpunk','minimal','royal','gaming','ocean','sunset','matrix','synthwave','forest','midnight','neon-pulse','aurora-wave','fire-storm','electric-blue','plasma-burst'].includes(id)) return 'core';
  if (['christmas','halloween','summer','autumn'].includes(id)) return 'seasonal';
  if (id.startsWith('frosted-') || id.startsWith('crystal-') || id.startsWith('smoke-') || id.startsWith('amber-glass') || id.startsWith('rose-glass')) return 'glass';
  if (['gold-luxe','platinum-elite','emerald-crown','sapphire-royal','obsidian-prestige'].includes(id)) return 'luxury';
  if (id.startsWith('glossy-')) return 'glossy';
  if (id.startsWith('matte-')) return 'matte';
  if (id.startsWith('anodized-')) return 'anodized';
  if (id.startsWith('metallic-')) return 'metallic';
  if (id.startsWith('vibrant-')) return 'vibrant';
  if (id.startsWith('vivid-')) return 'vivid';
  if (id.startsWith('fx-')) return 'fx';
  return 'core';
}

/** Pretty display name from id */
function formatName(id) {
  return id.replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
}

/** Build the full theme list with metadata */

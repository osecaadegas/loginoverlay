...
function getGuessSessionTotals(formData, slots) {
  ...
  const finalBal = formData.final_balance
    ? Number.parseFloat(formData.final_balance)
    : null;
  const totalBets = slots.reduce(
    (sum, slot) => sum + (Number.parseFloat(slot.bet_value) || 0),
    0,
  );
  const beMultiplier =
    startVal > 0 && totalBets > 0 && finalBal
      ? finalBal / startVal / totalBets
      : 1.0;
...
async function fetchSessionVotes(sessionId) {
  ...
  const slotsMap = Object.fromEntries(
    (slotsData || []).map((slot) => [slot.id, slot.slot_name]),
  );
...
export default function AdminPanel() { //NOSONAR - legacy admin screen split should be handled as a dedicated refactor
  ...
  const [newSlotBetValue, setNewSlotBetValue] = useState(1.0);
  ...
  const handleAddRole = async () => {
    ...
    if (editingUser.newRoleExpiryDays && editingUser.newRoleExpiryDays > 0) {
      const date = new Date();
      date.setDate(
        date.getDate() + Number.parseInt(editingUser.newRoleExpiryDays, 10),
      );
  ...
  const savePrize = async (e) => {
    ...
    try {
      ...
      if (editingPrize) {
        ...
        result = await supabase.from("daily_wheel_prizes").insert([prizeData]);
      }
  ...
  const filteredSlotCatalog = slotCatalog.filter((slot) => {
    ...
    const providerMatch = (slot.provider || "")
      .toLowerCase()
  ...
  const saveGuessSession = async (e) => {
    ...
    try {
      ...
      const { startVal, finalBal, totalBets, beMultiplier } =
        getGuessSessionTotals(guessSessionFormData, sessionSlotsInModal);
      ...
      if (sessionSlotsInModal.length > 0) {
        const slotsToInsert = sessionSlotsInModal.map((slot, index) => ({
          ...
          multiplier: slot.multiplier
            ? Number.parseFloat(slot.multiplier)
            : null,
          display_order: index,
        }));
      ...
      setSuccess(
        `Session ${editingGuessSession ? "updated" : "created"} successfully with ${sessionSlotsInModal.length} slots!`,
      );
  ...
  const saveSlot = async (e) => {
    ...
    try {
      const slotData = {
        ...
        bonus_win: slotFormData.bonus_win
          ? Number.parseFloat(slotFormData.bonus_win)
          : null,
        multiplier: slotFormData.multiplier
          ? Number.parseFloat(slotFormData.multiplier)
          : null,
      ...
      if (editingSlot) {
        ...
        result = await supabase.from("guess_balance_slots").insert([slotData]);
      }
  ...
  const generateTransferPassword = async () => {
    ...
    try {
      ...
      setSuccess(
        "Transfer password generated! It expires in 24 hours. Copy it now — you won't see it again.",
      );
      loadActiveTransferPassword();
    } catch (err) {
  ...
  const adminTabs = [
    ...
    },
    {
      ...
      shortLabel: "API Keys",
      kicker: "Access",
      description: "External integrations and tokens",
    },
  ];
    ...
    <div className="admin-panel admin-panel-modern">
      ...
      <div className="admin-dropdown-nav">
        ...
        <div className="admin-dropdown-wrapper admin-dropdown-wrapper-clean">
          ...
          <svg
            className="admin-dropdown-chevron"
            width="16"
            height="16"
            viewBox="0 0 24 24"
            fill="none"
        ...
        <>
          <StatsGrid columns={7}>
            ...
            <StatsCard
              ...
              value={
                users.filter((u) =>
                  u.roles?.some((r) => r.role === "slot_modder"),
                ).length
              }
              label="Slot Modders"
              color="warning"
            ...
            <StatsCard
              ...
              value={
                users.filter((u) =>
                  u.roles?.some((r) => r.role === "moderator"),
                ).length
              }
              label="Moderators"
              color="info"
            ...
            <StatsCard
              icon="💎"
              value={
                users.filter((u) => u.roles?.some((r) => r.role === "premium"))
                  .length
              }
              label="Premium"
              color="warning"
            ...
            <StatsCard
              ...
              value={
                users.filter((u) =>
                  u.roles?.some((r) => r.role === "affiliate"),
                ).length
              }
              label="Affiliates"
              color="primary"
              ...
              <div className="toolbar-left">
                ...
                <div className="filter-pills">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setRoleFilter(roleFilter === "admin" ? null : "admin")
                    }
                  >
                    <span className="pill-dot admin"></span> Admins
                  </button>
                  <button
                    className={`filter-pill ${roleFilter === "premium" ? "active" : ""}`}
                    onClick={() =>
                      setRoleFilter(roleFilter === "premium" ? null : "premium")
                    }
                  >
                    <span className="pill-dot premium"></span> Premium
                  </button>
                  <button
                    className={`filter-pill ${roleFilter === "slot_modder" ? "active" : ""}`}
                    onClick={() =>
                      setRoleFilter(
                        roleFilter === "slot_modder" ? null : "slot_modder",
                      )
                    }
                  >
                    <span className="pill-dot modder"></span> Modders
                  </button>
                  <button
                    className={`filter-pill ${roleFilter === "affiliate" ? "active" : ""}`}
                    onClick={() =>
                      setRoleFilter(
                        roleFilter === "affiliate" ? null : "affiliate",
                      )
                    }
                  >
                    <span className="pill-dot affiliate"></span> Affiliates
                  </button>
                </div>
              </div>
            ...
            <div className="data-table-wrapper">
                  ...
                  {users
                    ...
                    .map((user) => {
                      const significantRoles =
                        user.roles?.filter((r) => r.role !== "user") || [];
                      return (
                        <tr
                          key={user.id}
                        ...
                        >
                          <td className="col-user">
                            <div className="user-cell">
                              <div
                                className={`avatar avatar-${user.provider?.toLowerCase() || "email"}`}
                              >
                            ...
                            <div className="roles-list">
                              {significantRoles.length > 0 ? (
                                significantRoles.map((roleObj) => (
                                  <span
                                    key={`${user.id}-${roleObj.role}`}
                                    className={`role-tag role-${roleObj.role}`}
                                  >
              ...
              {users.filter((user) => {
                ...
                const matchesSearch =
                  ...
                  user.provider_username?.toLowerCase().includes(search);
                const matchesRole =
                  !roleFilter || user.roles?.some((r) => r.role === roleFilter);
                return matchesSearch && matchesRole;
              }).length === 0 && (
            ...
            {editingUser && (
              <div className="panel-content">
                ...
                <div className="panel-user-header">
                  <div
                    className={`panel-avatar avatar-${editingUser.provider?.toLowerCase() || "email"}`}
                  >
                  ...
                  <div className="panel-user-info">
                    ...
                    <div className="panel-user-meta">
                      <span
                        className={`status-dot ${editingUser.is_active ? "active" : "inactive"}`}
                      >
                        <span className="dot"></span>
                ...
                <div className="panel-section">
                  ...
                  <div className="roles-manager">
                    {(editingUser.roles || [])
                      ...
                      .map((roleObj) => (
                        <div
                          key={`${editingUser.id}-${roleObj.role}`}
                          className={`role-chip role-${roleObj.role}`}
                        >
                    ...
                    {(!editingUser.roles ||
                      editingUser.roles.filter((r) => r.role !== "user")
                        .length === 0) && (
                    ...
                    {editingUser.newRole === "moderator" && (
                      <div className="permissions-box">
                        ...
                        <div className="permissions-list">
                          {Object.entries(MODERATOR_PERMISSIONS).map(
                            ([key, description]) => (
                      ...
                      <div className="add-role-actions">
                        <input
                          ...
                          onChange={(e) =>
                            setEditingUser({
                              ...editingUser,
                              newRoleExpiryDays: e.target.value,
                            })
                          }
                          min="1"
      ...
      <Show when={activeTab === "offers"}>
        <div className="offers-management">
          <div className="offers-header">
            ...
            <div
              className="offers-header-actions"
              style={{
                display: "flex",
                gap: 8,
                alignItems: "center",
              ...
              <div style={{ display: "flex", gap: 4 }}>
                <button
                  ...
                  style={{
                    ...
                    color: offerSubTab === "cards" ? "#fff" : "#94a3b8",
                  }}
                >
                  📰 Marketplace
                </button>
                <button
                  onClick={() => {
                    setOfferSubTab("analytics");
                    if (offerClicks.length === 0) loadOfferClicks();
                  }}
                  style={{
                    padding: "6px 14px",
                    borderRadius: 6,
                    ...
                    background:
                      offerSubTab === "analytics"
                        ? "#6366f1"
                        : "rgba(255,255,255,0.08)",
                    color: offerSubTab === "analytics" ? "#fff" : "#94a3b8",
                  }}
                >
                  📊 Click Analytics
                </button>
            ...
            <>
              <div className="offers-grid">
                {offers
                  ...
                  .map((offer) => (
                    <div
                      key={offer.id}
                      className={`offer-admin-card ${!offer.is_active ? "inactive" : ""}`}
                    >
                      <div className="offer-admin-image">
                        ...
                        {(offer.is_featured || offer.badge) && (
                          <span
                            className={`offer-badge ${offer.is_featured ? "featured" : offer.badge_class}`}
                          >
                      ...
                      <div className="offer-admin-content">
                        ...
                        <div className="offer-stats">
                          ...
                          <span>
                            {offer.visibility ||
                              (offer.is_premium ? "premium" : "public")}
                          </span>
                        </div>
                        <div className="offer-stats">
                          <span>
                            {offer.cpa_amount
                              ? `${offer.cpa_currency || "EUR"} ${offer.cpa_amount}`
                              : "CPA not set"}
                          </span>
                          <span>
                            {offer.revenue_share_percent
                              ? `${offer.revenue_share_percent}% RevShare`
                              : "RevShare not set"}
                          </span>
                        ...
                        <div className="offer-admin-actions">
                          ...
                          <button
                            onClick={() =>
                              toggleOfferActive(offer.id, offer.is_active)
                            }
              ...
              {offers.length === 0 && (
                <div className="no-offers">
                  <p>
                    No partnerships yet. Create your first marketplace deal!
                  </p>
                </div>
              )}
            </>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              {!clicksLoading && offerClicks.length === 0 && (
                <p
                  style={{ color: "#94a3b8", textAlign: "center", padding: 40 }}
                >
                  No clicks recorded yet. Clicks will appear here once viewers
                  start clicking offer cards.
                </p>
              )}
              {!clicksLoading && offerClicks.length > 0 && (
                <>
                  ...
                  {(() => {
                    ...
                    const sorted = Object.entries(byOffer).sort(
                      (a, b) => b[1] - a[1],
                    );
                    const sortedCountries = Object.entries(byCountry).sort(
                      (a, b) => b[1] - a[1],
                    );
                    return (
                      <>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Full Click History ({offerClicks.length} total)
                      </div>
                      <div
                        style={{
                          display: "flex",
                        ...
                        {["all", "today", "7d", "30d"].map((f) => (
                          <button
                            ...
                            style={{
                              ...
                              color: clickDateFilter === f ? "#fff" : "#94a3b8",
                            }}
                          >
                            {
                              {
                          ...
                          {(() => {
                            ...
                            const filtered = offerClicks.filter((c) => {
                              ...
                              if (clickDateFilter !== "all") {
                                ...
                                if (
                                  clickDateFilter === "today" &&
                                  created.toDateString() !== now.toDateString()
                                )
                                  return false;
                                if (
                                  clickDateFilter === "7d" &&
                                  now - created > 7 * 86400000
                                )
                                  return false;
                                if (
                                  clickDateFilter === "30d" &&
                                ...
                                <React.Fragment key={click.id}>
                                  <tr
                                    ...
                                    style={{
                                      ...
                                      cursor: "pointer",
                                      background: getOfferClickRowBackground(
                                        isExpanded,
                                        idx,
                                      ),
                                    }}
                                      ...
                                      {click.ip_address ? (
                                        ...
                                        >
                                          ...
                                          {click.ip_address.includes(":") ? (
                                            <span
                                              title="IPv6 address (won't match affiliate dashboards)"
                                              style={{
                                                padding: "2px 6px",
                                                borderRadius: 4,
                                                fontSize: 9,
                                            ...
                                            <span
                                              title="IPv4 address (matches affiliate dashboards)"
                                              style={{
                                                padding: "2px 6px",
                                                borderRadius: 4,
                                                fontSize: 9,
        ...
        onSave={async (formData) => {
          try {
            ...
            if (editingOffer) {
              ...
              const { error } = await supabase.from("casino_offers").insert([
                {
                  ...payload,
              ...
              {wheelPrizes.map((prize) => (
                <div key={prize.id} className="prize-card">
                  ...
                  <div className="prize-actions">
                    <button
                      onClick={() =>
                        togglePrizeActive(prize.id, prize.is_active)
                      }
              ...
              <div style={{ flex: 1, overflow: "auto", padding: "0" }}>
                <div className="form-section">
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      <label htmlFor="wheel-prize-bg-color">
                        Background Color *
                      </label>
                      <input
                    ...
                    <div className="form-group">
                      <label htmlFor="wheel-prize-text-color">
                        Text Color *
                      </label>
                      <input
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      <label htmlFor="wheel-prize-points">
                    ...
                    <div className="form-group">
                      <label htmlFor="wheel-prize-probability">
                        Probability Weight *
                      </label>
                      <input
                  ...
                  <div className="form-group">
                    <label htmlFor="wheel-prize-display-order">
                      Display Order
                    </label>
                    <input
          ...
          <div className="guess-balance-admin-section">
            ...
            <div className="gtb-transfer-section">
              <div className="gtb-transfer-header">
                ...
                <p className="gtb-transfer-desc">
                  Generate a password to allow transferring bonuses from the
                  Overlay Bonus Hunt tracker into a new GTB session.
                </p>
              </div>
                ...
                {activeTransferPassword ? (
                  <div className="gtb-transfer-active">
                    ...
                    <span className="gtb-transfer-expires">
                      Expires:{" "}
                      {new Date(
                        activeTransferPassword.expires_at,
                      ).toLocaleString()}
                    </span>
                ...
                {transferPassword && showTransferPassword && (
                  <div className="gtb-transfer-reveal">
                    <div className="gtb-transfer-password-box">
                      <span className="gtb-transfer-password-label">
                        Your transfer password:
                      </span>
                      <code className="gtb-transfer-password-value">
                        {transferPassword}
                      </code>
                      <button
                        className="btn-copy"
                    ...
                    <p className="gtb-transfer-warning">
                      ⚠️ Copy this password now! It will not be shown again. Use
                      it in the Bonus Hunt tracker to send bonuses to GTB.
                    </p>
                    <button
            ...
            <div className="sessions-list">
              {guessBalanceSessions.length === 0 ? (
                <div className="empty-state">
                  <p>
                    No sessions yet. Create your first Guess the Balance
                    session!
                  </p>
                </div>
              ) : (
                  ...
                  {guessBalanceSessions.map((session) => (
                    ...
                    >
                      ...
                      {session.casino_brand && (
                        <div className="casino-info">
                          {session.casino_image_url && (
                            <img
                              src={session.casino_image_url}
                              alt={session.casino_brand}
                              className="casino-logo-small"
                            />
                          )}
                      ...
                      <div className="session-flags">
                        <span
                          className={`flag ${session.is_guessing_open ? "active" : ""}`}
                        >
                        ...
                        <span
                          className={`flag ${session.reveal_answer ? "revealed" : ""}`}
                        >
                      ...
                      <div className="session-actions">
                        ...
                        {session.status === "active" &&
                          session.final_balance && (
                            <ConfirmButton
                              onConfirm={() =>
                                endGuessSessionAndCalculateWinner(session.id)
                              }
                              confirmText="End?"
                              className="btn-end"
                ...
                <div className="slots-admin-grid">
                  {guessBalanceSlots.length === 0 ? (
                    ...
                    guessBalanceSlots.map((slot, index) => (
                      <div
                        key={slot.id}
                        className={`slot-admin-card ${slot.is_super ? "super" : ""}`}
                      >
                        <div className="slot-number">#{index + 1}</div>

                        {slot.slot_image_url ? (
                          <img
                            src={slot.slot_image_url}
                            alt={slot.slot_name}
                            className="slot-admin-image"
                          />
                        ) : (
                        ...
                        <div className="slot-admin-info">
                          ...
                          <div className="slot-stats">
                            <span>
                              Bet: €
                              {Number.parseFloat(slot.bet_value || 0).toFixed(
                                2,
                              )}
                            </span>
                            {slot.is_super && (
                              <span className="super-tag">⭐ SUPER</span>
                            )}
                          </div>
                          {slot.bonus_win !== null && (
                            <div className="slot-results">
                              <span>
                                Win: €
                                {Number.parseFloat(slot.bonus_win || 0).toFixed(
                                  2,
                                )}
                              </span>
                              {slot.multiplier && (
                                <span>{slot.multiplier}x</span>
                              )}
                            </div>
                          )}
                        </div>
            ...
            footer={
              <>
                <button
                  ...
                  onClick={() => setShowGuessBalanceModal(false)}
                >
                  Cancel
                </button>
                <button
            ...
            <form className="guess-session-form">
              ...
              <div className="gtb-sidepanel-transfer">
                <div className="gtb-sidepanel-transfer-header">
                  ...
                  {activeTransferPassword ? (
                    <span className="gtb-sp-badge gtb-sp-badge--active">
                      Active
                    </span>
                  ) : (
                    <span className="gtb-sp-badge gtb-sp-badge--none">
                      None
                    </span>
                  )}
                </div>
                <p className="gtb-sidepanel-transfer-desc">
                  Generate a password to send bonuses from the Overlay Bonus
                  Hunt into a GTB session.
                </p>
                <div className="gtb-sidepanel-transfer-controls">
                  <button
                    ...
                    disabled={transferPasswordLoading}
                  >
                    {getTransferPasswordButtonLabel(
                      transferPasswordLoading,
                      activeTransferPassword,
                    )}
                  </button>
              ...
              <div className="form-row two-cols">
                <div className="form-group">
                  <label htmlFor="guess-session-start-value">
                    Start Value (€)
                  </label>
                  <input
                ...
                <div className="form-group">
                  <label htmlFor="guess-session-final-balance">
                    Final Balance (€)
                  </label>
                  <input
              ...
              <div className="form-row two-cols">
                <div className="form-group">
                  <label htmlFor="guess-session-amount-expended">
                    Amount Expended (€)
                  </label>
                  <input
                    ...
                    type="number"
                    step="0.01"
                    value={sessionSlotsInModal
                      .reduce(
                        (sum, s) => sum + (Number.parseFloat(s.bet_value) || 0),
                        0,
                      )
                      .toFixed(2)}
                    readOnly
                  ...
                  <small className="form-hint">
                    Auto-calculated from slots
                  </small>
                </div>
                <div className="form-group">
                  <label htmlFor="guess-session-be-multiplier">
                    BE Multiplier (x)
                  </label>
                  <input
                    ...
                    value={(() => {
                      const startVal =
                        Number.parseFloat(guessSessionFormData.start_value) ||
                        0;
                      const finalBal =
                        Number.parseFloat(guessSessionFormData.final_balance) ||
                        0;
                      const totalBets = sessionSlotsInModal.reduce(
                        (sum, s) => sum + (Number.parseFloat(s.bet_value) || 0),
                        0,
                      );
              ...
              <div className="form-row">
                <div className="form-group">
                  <label htmlFor="guess-session-casino-brand">
                    Casino Brand
                  </label>
                  <input
                ...
                <div className="form-group">
                  <label htmlFor="guess-session-casino-logo">
                    Casino Logo URL
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setGuessSessionFormData({
                        ...guessSessionFormData,
                        casino_image_url: e.target.value,
                      })
                    }
                    placeholder="https://..."
                  />
                ...
                <label className="checkbox-label">
                  <input
                    ...
                    onChange={(e) =>
                      setGuessSessionFormData({
                        ...guessSessionFormData,
                        is_guessing_open: e.target.checked,
                      })
                    }
                  />
                ...
                <label className="checkbox-label">
                  <input
                    ...
                    onChange={(e) =>
                      setGuessSessionFormData({
                        ...guessSessionFormData,
                        reveal_answer: e.target.checked,
                      })
                    }
                  />
              ...
              <div className="slot-picker-section">
                <div className="slot-picker-controls">
                  <div className="form-group slot-search-group">
                    <label htmlFor="guess-session-slot-search">
                      Search Slots
                    </label>
                    <input
                  ...
                  <div className="slot-picker-row">
                    <div className="form-group bet-group">
                      <label htmlFor="guess-session-new-slot-bet">
                        Bet Value (€)
                      </label>
                      <input
                ...
                {slotSearchQuery && (
                  <div className="slot-catalog-results">
                    ...
                    >
                      Searching in {slotCatalog.length} slots • Found{" "}
                      {filteredSlotCatalog.length} matches
                    </div>
                        ...
                        {filteredSlotCatalog.slice(0, 20).map((slot) => (
                          ...
                          >
                            <img
                              src={slot.image}
                              alt={slot.name}
                              className="slot-catalog-image"
                            />
                            <div className="slot-catalog-info">
                              <span className="slot-catalog-name">
                                {slot.name}
                              </span>
                              <span className="slot-catalog-provider">
                                {slot.provider}
                              </span>
                            </div>
                            <span className="add-slot-btn" aria-hidden="true">
                              +
                            </span>
                          </button>
                        ))}
                  ...
                  <div className="added-slots-grid">
                    {sessionSlotsInModal.map((slot, index) => (
                      <div
                        ...
                        className={`added-slot-item ${slot.is_super ? "super" : ""}`}
                      >
                        ...
                        {slot.slot_image_url ? (
                          <img
                            src={slot.slot_image_url}
                            alt={slot.slot_name}
                            className="added-slot-image"
                          />
                        ) : (
                          <div className="added-slot-placeholder">🎰</div>
                        )}
                        <div className="added-slot-info">
                          <span className="added-slot-name">
                            {slot.slot_name}
                          </span>
                          <span className="added-slot-provider">
                            {slot.provider}
                          </span>
                        </div>
                        <span className="added-slot-bet">
                          €{Number.parseFloat(slot.bet_value || 0).toFixed(2)}
                        </span>
                        {slot.is_super && (
                          <span className="super-badge">⭐</span>
                        )}
                        <button
                          type="button"
            ...
            <form className="slot-form">
              ...
              <div className="form-group">
                ...
                {slotFormData.slot_image_url && (
                  <img
                    src={slotFormData.slot_image_url}
                    alt="Preview"
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  <label htmlFor="guess-slot-display-order">
                    Display Order
                  </label>
                  <input
              ...
              <div className="form-section-title">
                🏆 Results (optional - fill when bonus opens)
              </div>
              <div className="form-row">
                <div className="form-group">
            ...
            footer={
              <>
                <button
                  ...
                  onClick={() =>
                    saveSlotResult(guessBalanceSlots[currentSlotIndex])
                  }
                >
                  💾 Save & Continue
                </button>
          ...
          >
            <div className="slot-results-content">
              ...
              <div className="slot-results-progress">
                <span>
                  Slot {currentSlotIndex + 1} of {guessBalanceSlots.length}
                </span>
                <div className="progress-bar">
                  <div
                    className="progress-fill"
                    style={{
                      width: `${((currentSlotIndex + 1) / guessBalanceSlots.length) * 100}%`,
                    }}
                  />
                </div>
                <span className="completed-count">
                  {
                    guessBalanceSlots.filter(
                      (s) => s.bonus_win !== null && s.bonus_win !== "",
                    ).length
                  }{" "}
                  completed
                </span>
                  ...
                  <div className="current-slot-display">
                    <div className="slot-image-large">
                      {currentSlot.slot_image_url ? (
                        <img
                          src={currentSlot.slot_image_url}
                          alt={currentSlot.slot_name}
                        />
                      ) : (
                        <div className="no-image-large">🎰</div>
                      )}
                      {currentSlot.is_super && (
                        <span className="super-badge-large">⭐ SUPER</span>
                      )}
                    </div>
                    ...
                    <div className="slot-info-large">
                      ...
                      {currentSlot.provider && (
                        <p className="slot-provider">{currentSlot.provider}</p>
                      )}
                      <p className="slot-bet">
                        Bet: €{currentSlot.bet_value || "0.00"}
                      </p>
                    </div>
                    ...
                    <div className="result-inputs">
                      <div className="input-group">
                        <label
                          htmlFor={`slot-result-bonus-win-${currentSlot.id}`}
                        >
                          💰 Bonus Win (€)
                        </label>
                        <input
                      ...
                      {currentSlot.bet_value && currentSlot.bonus_win && (
                        <div className="auto-multiplier">
                          📊 Multiplier:{" "}
                          {(
                            Number.parseFloat(currentSlot.bonus_win) /
                            Number.parseFloat(currentSlot.bet_value)
                          ).toFixed(2)}
                          x
                        </div>
                      )}
              ...
              {sessionGuesses.length === 0 ? (
                ...
                <>
                  <div className="guesses-summary">
                    <span className="total-guesses">
                      Total: {sessionGuesses.length} guesses
                    </span>
                  </div>
                    ...
                    <table className="guesses-table">
                      ...
                      <tbody>
                        {sessionGuesses.map((guess, index) => (
                          <tr
                            key={guess.id}
                            className={guess.is_winner ? "winner-row" : ""}
                          >
                            <td>{index + 1}</td>
                            <td>{guess.display_name || "Anonymous"}</td>
                            <td className="guess-amount">
                              €
                              {Number.parseFloat(guess.guessed_balance).toFixed(
                                2,
                              )}
                            </td>
                            <td className="guess-time">
                              {new Date(guess.guessed_at).toLocaleString()}
                            </td>
                ...
                <>
                  <div className="votes-summary">
                    <span className="total-votes">
                      Total: {sessionVotes.length} votes
                    </span>
                    <span className="best-votes">
                      👍 Best:{" "}
                      {
                        sessionVotes.filter((v) => v.vote_type === "best")
                          .length
                      }
                    </span>
                    <span className="worst-votes">
                      👎 Worst:{" "}
                      {
                        sessionVotes.filter((v) => v.vote_type === "worst")
                          .length
                      }
                    </span>
                  </div>
                    ...
                    <table className="votes-table">
                      ...
                      <tbody>
                        {sessionVotes.map((vote, index) => (
                          <tr
                            key={vote.id}
                            className={`vote-row ${vote.vote_type}`}
                          >
                            <td>{index + 1}</td>

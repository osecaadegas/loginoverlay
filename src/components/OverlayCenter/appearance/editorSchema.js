...
export function normalizeSimpleSettings(settings = {}) {
  ...
  return {
    ...next,
    material: materialIds.has(next.material)
      ? next.material
    ...
    accentColor: normalizeHexColor(
      next.accentColor,
      DEFAULT_SIMPLE_SETTINGS.accentColor,
    ),
    useSecondColor: !!next.useSecondColor,
    shape: shapeIds.has(next.shape)
      ? next.shape
      : DEFAULT_SIMPLE_SETTINGS.shape,
    density: densityIds.has(next.density)
      ? next.density
      : DEFAULT_SIMPLE_SETTINGS.density,
    scale: clamp(next.scale, 0.75, 1.5),
    fontFamily: fontValues.has(next.fontFamily)
      ? next.fontFamily
      : DEFAULT_SIMPLE_SETTINGS.fontFamily,
    textSize: textSizeIds.has(next.textSize)
      ? next.textSize
      : DEFAULT_SIMPLE_SETTINGS.textSize,
    boldText: !!next.boldText,
    carouselSpeed: motionSpeedIds.has(next.carouselSpeed)
      ? next.carouselSpeed
      : DEFAULT_SIMPLE_SETTINGS.carouselSpeed,
    carouselDirection: ["left", "right"].includes(next.carouselDirection)
      ? next.carouselDirection
      : DEFAULT_SIMPLE_SETTINGS.carouselDirection,
    ...
    animationSpeed: motionSpeedIds.has(next.animationSpeed)
      ? next.animationSpeed
      : DEFAULT_SIMPLE_SETTINGS.animationSpeed,
    animationIntensity: ["subtle", "normal", "strong"].includes(
      next.animationIntensity,
    )
      ? next.animationIntensity
      : DEFAULT_SIMPLE_SETTINGS.animationIntensity,
    shadowStrength: strengthIds.has(next.shadowStrength)
      ? next.shadowStrength
      : DEFAULT_SIMPLE_SETTINGS.shadowStrength,
    glowStrength: strengthIds.has(next.glowStrength)
      ? next.glowStrength
      : DEFAULT_SIMPLE_SETTINGS.glowStrength,
    imageVisibility: ["show", "hidden"].includes(next.imageVisibility)
      ? next.imageVisibility
      : DEFAULT_SIMPLE_SETTINGS.imageVisibility,
    imageSize: imageSizeIds.has(next.imageSize)
      ? next.imageSize
      : DEFAULT_SIMPLE_SETTINGS.imageSize,
    imageShape: imageShapeIds.has(next.imageShape)
      ? next.imageShape
      : DEFAULT_SIMPLE_SETTINGS.imageShape,
    imageFit: ["cover", "contain"].includes(next.imageFit)
      ? next.imageFit
      : DEFAULT_SIMPLE_SETTINGS.imageFit,
    barHeight:
      next.barHeight != null &&
      next.barHeight !== "" &&
      Number.isFinite(Number(next.barHeight))
        ? clamp(Number(next.barHeight), 32, 220)
        : null,
    maxWidth:
      next.maxWidth != null &&
      next.maxWidth !== "" &&
      Number.isFinite(Number(next.maxWidth))
        ? clamp(Number(next.maxWidth), 240, 1920)
        : null,
    musicDisplayStyle: [
      "text",
      "pill",
      ...
      "minimal",
      "wave",
    ].includes(next.musicDisplayStyle)
...
function buildSimpleBase(settings) {
  ...
  const accent = deriveAccent(
    ...
    settings.useSecondColor,
  );
  const shape =
    SIMPLE_SHAPES.find((item) => item.id === settings.shape) ||
    SIMPLE_SHAPES[2];
  const density =
    SIMPLE_DENSITIES.find((item) => item.id === settings.density) ||
    SIMPLE_DENSITIES[1];
  const textSize =
    SIMPLE_TEXT_SIZES.find((item) => item.id === settings.textSize) ||
    SIMPLE_TEXT_SIZES[1];
...
export function generateGradientTheme(settings) {
  const base = buildSimpleBase(settings);
  const second = settings.useSecondColor
    ? base.accent
    : mixHex(base.primary, "#8b5cf6", 0.45);
...
export function generateNeonTheme(settings) {
  ...
  return {
    ...
    borders: {
      enabled: true,
      width: 1,
      color: toRgba(base.primary, 0.68),
      style: "solid",
    },
    effects: {
      shadowEnabled: true,
...
export function generateMinimalTheme(settings) {
  ...
  return {
    ...
    borders: {
      enabled: true,
      width: 1,
      color: toRgba(base.primary, 0.24),
      style: "solid",
    },
...
export function generateSimpleAppearance(input = {}) {
  ...
  const generated = (
    materialGenerators[settings.material] || generateMatteTheme
  )(settings);
  const base = buildSimpleBase(settings);
  const textColor = generated.colors?.text || getReadableTextColor(base.dark);
  const contrastBackground =
    generated.surfaces?.containerBg === "transparent" ? "#020617" : base.dark;
  ...
  return deepMergeSimple(generated, {
    ...
    controls: {
      ...
      primaryText:
        generated.colors?.buttonText || getReadableTextColor(base.primary),
...
export const CONTROL_DEFINITIONS = {
  ...
  opacity: {
    ...
    simple: true,
    group: "Effects",
    help: "How see-through this part is.",
  },
  shadowBlur: {
    id: "shadowBlur",
  ...
  fxParticles: {
    ...
    label: "Particles",
    type: "select",
    options: ["none", "orbs", "fireflies", "bokeh", "snow", "rain"],
    simple: true,
    group: "Effects",
  },
  fxParticleColor: {
  ...
  fxGlimpse: {
    ...
    label: "Light effect",
    type: "select",
    options: ["none", "sweep", "pulse", "flicker"],
    simple: true,
    group: "Effects",
  },
  fxGlimpseColor: {
  ...
  animation: {
    ...
    label: "Animation",
    type: "segmented",
    options: ["none", "fade", "slide", "scale", "pulse", "glow"],
    group: "Animation",
  },
  duration: {
    id: "duration",
...
const TEXT_IDS =
  /title|heading|header|subtitle|label|value|text|name|footer|caption|question|status|percentage|number|timer|count|amount|requester|description/i;
const SURFACE_IDS =
  /container|background|card|row|section|box|cell|button|panel|footer|header|wrapper|shell/i;
...
const ADVANCED_CORNER_CONTROLS = [
  "topLeft",
  "topRight",
  "bottomRight",
  "bottomLeft",
...
export const BUILT_IN_STYLE_PRESETS = [
  ...
  },
  {
    ...
    appearance: {
      ...
      surfaces: {
        containerBg: "rgba(248,250,252,0.94)",
  ...
  },
  {
    ...
    appearance: {
      ...
      surfaces: {
        containerBg: "rgba(2,6,23,0.72)",
        cardBg: "rgba(2,6,23,0.36)",
        opacity: 0.88,
        blur: 0,
        padding: 8,
        gap: 6,
...
export function getWidgetCategory(widgetOrType) {
  const type =
    typeof widgetOrType === "string"
      ? widgetOrType
      : widgetOrType?.widget_type || widgetOrType?.type || "";
  const def = getWidgetDef(type);
  const haystack =
    `${type} ${def?.label || ""} ${def?.category || ""}`.toLowerCase();
  if (
    /bonus.?hunt|current.?slot|single.?slot|rtp|bonus.?buy|slot.?manager/.test(
      haystack,
    )
  )
    return "bonus_hunt";
  ...
  if (
    /game|bets|coin|wheel|prediction|keno|mines|slotmachine|picker/.test(
      haystack,
    )
  )
    return "games";
...
export function getWidgetIcon(widgetOrType) {
  const type =
    typeof widgetOrType === "string" ? widgetOrType : widgetOrType?.widget_type;
  const def = getWidgetDef(type);
...
export function elementSupportsControl(element = {}, controlId) {
  const explicitControls =
    Array.isArray(element.controls) && element.controls.length
      ? element.controls
      : null;
  ...
  if (kind === "text")
    return (
      TEXT_CONTROLS.includes(controlId) ||
      ["opacity", "shadowBlur", "shadowOpacity"].includes(controlId)
    );
  if (kind === "surface")
    return (
      SURFACE_CONTROLS.includes(controlId) || SIZE_CONTROLS.includes(controlId)
    );
  if (kind === "image")
    return (
      IMAGE_CONTROLS.includes(controlId) || SIZE_CONTROLS.includes(controlId)
    );
  if (kind === "progress")
    return (
      PROGRESS_CONTROLS.includes(controlId) || SIZE_CONTROLS.includes(controlId)
    );
...
export function getElementControlGroups(element = {}, mode = "simple") {
  ...
  const explicitControls = Array.isArray(element.controls)
    ? element.controls
    : [];
  ...
  if (isWidgetAppearanceV2Enabled(widgetType)) {
    ...
    if (v2Elements.length) {
      return v2Elements.map((element) => ({
        ...
        controls:
          element.controls ||
          getElementControlGroups(element, "advanced").flatMap((group) =>
            group.controls.map((control) => control.id),
          ),
      }));
    }
  }
  const elements = getWidgetSubElementDefinitions(widgetType).map(
    (element) => ({
      ...element,
      label: getFriendlyElementLabel(element.id, element.label),
      kind: inferElementKind(element),
      controls: getElementControlGroups(element, "advanced").flatMap((group) =>
        group.controls.map((control) => control.id),
      ),
    }),
  );
...
export function getElementValue(
  ...
  const definition = COMMON_APPEARANCE_PROPERTY_DEFINITIONS.find(
    (item) => item.path === property,
  );
  const groupedPath = definition?.path || property;
  const value = getByPath(
    appearance,
    `${targetRoot}.elements.${elementId}.${groupedPath}`,
  );
  if (value !== undefined) return value;
  const legacy = getByPath(
    appearance,
    `${targetRoot}.subElements.${elementId}.${property}`,
  );
...
export function validateEditorValue(control, value) {
  ...
  if (control.type === "color") {
    ...
    if (
      /^#([0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(next) ||
      /^rgba?\(/i.test(next) ||
      /^hsla?\(/i.test(next) ||
      next === "transparent"
    )
  ...
  if (control.type === "range" || control.type === "number") {
    ...
    return Math.min(
      Number(control.max ?? next),
      Math.max(Number(control.min ?? next), next),
    );
  }

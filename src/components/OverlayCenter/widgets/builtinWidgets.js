...
registerWidget({
  ...
  defaults: {
    ...
    srSeEnabled: false,
    srSeCost: 100,
    srMsgAccepted: '🎰 Added "{slot}" to the queue (requested by {user})',
    srMsgAcceptedCost: '🎰 Added "{slot}" to the queue ({user} — {cost} points deducted)',
    srMsgNotEnough: '❌ {user}, you need {cost} points to request a slot (you have {points}).',
    srMsgDuplicate: '⚠️ {user}, "{slot}" is already in the queue (requested by {by}). No points taken!',
    srMsgRejected: '🚫 {user}, your request for "{slot}" was rejected. {refund}',
...
registerWidget({
  ...
  defaults: {
    ...
    betMinAmount:  1,
    betMaxAmount:  0,
    betMsgPlaced: '@{user}, your {amount} point bet on {option} is in.',
    betMsgPlacedSe: '@{user}, your {amount} point bet on {option} is in. Points deducted.',
    betMsgNoPoints: '@{user}, you have {balance} points and tried to bet {amount}.',
    betMsgAlreadyBet: '@{user}, you already have a bet in this round.',
    betMsgNotOpen: '@{user}, bets are closed right now.',
    betMsgWinner: '{option} wins. {winners} winner(s), {total} points paid out.',

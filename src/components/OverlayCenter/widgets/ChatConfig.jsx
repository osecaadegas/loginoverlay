...
export default function ChatConfig({ config, onChange }) {
  ...
  const deletePreset = (name) =>
    set(
      "chatPresets",
      (c.chatPresets || []).filter((p) => p.name !== name),
    );
      ...
      {activeTab === "platforms" && (
        <div className="nb-section">
          ...
          <p className="oc-config-hint" style={{ marginBottom: 8 }}>
            Channel names are managed in your <b>Profile</b>. Click{" "}
            <b>Sync All</b> there to update.
          </p>
          ...
          <div className="ov-chat-cfg-platform">
            <label className="ov-chat-cfg-platform-header">
              <span
                className="ov-chat-cfg-platform-dot"
                style={{
                  background:
                    c.twitchChannel || autoChannel ? "#64748b" : "#333",
                }}
              />
              <span>Twitch</span>
          ...
          <div className="ov-chat-cfg-platform">
            <label className="ov-chat-cfg-platform-header">
              <span
                className="ov-chat-cfg-platform-dot"
                style={{ background: c.youtubeVideoId ? "#ef4444" : "#333" }}
              />
              <span>YouTube</span>
              {c.youtubeVideoId ? (
                <span
          ...
          <div className="ov-chat-cfg-platform">
            <label className="ov-chat-cfg-platform-header">
              <span
                className="ov-chat-cfg-platform-dot"
                style={{ background: c.kickChannelId ? "#22c55e" : "#333" }}
              />
              <span>Kick</span>
              {c.kickChannelId ? (
                <span
          ...
          {!(c.twitchChannel || autoChannel) &&
            ...
            !c.youtubeVideoId && (
              ...
              >
                ⚠️ No platforms configured — go to Profile and add your
                channels, then Sync.
              </p>
            )}
          ...
          <label className="ov-chat-cfg-platform-header" style={{ gap: 8 }}>
            <input
              type="checkbox"
              checked={!!c.useNativeColors}
              onChange={(e) => set("useNativeColors", e.target.checked)}
            />
          ...
          <label className="ov-chat-cfg-platform-header" style={{ gap: 8 }}>
            <input
              type="checkbox"
              checked={c.showRaidAvatar !== false}
              onChange={(e) => set("showRaidAvatar", e.target.checked)}
            />
            <span>Show Raider's Avatar</span>
          </label>
          <SliderField
            label="Max Messages"
            value={c.maxMessages ?? 50}
            min={5}
            max={200}
            step={5}
            unit=""
                ...
                <div key={p.name} className="nb-preset-pill">
                  <div className="nb-preset-pill__info">
                    ...
                    <div className="nb-preset-pill__swatches">
                      {["bgColor", "headerBg", "textColor", "borderColor"].map(
                        (k) =>
                          p.values[k] ? (
                            <span
                              key={k}

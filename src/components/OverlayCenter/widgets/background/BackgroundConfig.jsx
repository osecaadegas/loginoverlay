...
export default function BackgroundConfig({ config, onChange }) {
  ...
  const deletePreset = (name) =>
    set(
      "bgPresets",
      (c.bgPresets || []).filter((p) => p.name !== name),
    );
  ...
  const fxQuickPresets = [
    ...
    },
    {
      name: "✨ Fireflies",
      values: {
        fxParticles: "fireflies",
        fxParticleColor: "#fbbf24",
        fxParticleCount: 15,
        fxParticleSpeed: 30,
        fxParticleSize: 40,
      },
    ...
    {
      name: "💨 Heavy Smoke",
      values: { fxFog: "heavy", fxFogColor: "#000000" },
    },
    {
      name: "🔦 Sweep",
      values: {
        ...
        <div className="nb-section">
          ...
          <div className="oc-bg-texture-grid">
            {TEXTURE_OPTIONS.map((t) => (
              ...
              >
                <span className="oc-bg-texture-icon">
                  {t.label.split(" ")[0]}
                </span>
                <span className="oc-bg-texture-name">
          ...
          {[
            ...
            "diagonal",
            "conic",
          ].includes(c.textureType || "gradient") && (
            <SliderField
              label="Angle"
          ...
          {["dots", "grid", "diagonal"].includes(c.textureType) && (
            <SliderField
              label="Pattern Size"
              value={c.patternSize ?? 20}
              onChange={(v) => set("patternSize", v)}
              min={4}
              max={80}
              suffix="px"
            />
        ...
        <div className="nb-section">
          ...
          <div className="oc-bg-quick-grid">
            {fxQuickPresets.map((p) => (
              <button
                key={p.name}
                className="oc-bg-quick-btn"
                onClick={() => setMulti(p.values)}
              >
                {p.name}
              </button>
            ))}
          ...
          <label className="nb-field">
            <span>Type</span>
            <select
              value={c.fxParticles || "none"}
              onChange={(e) => set("fxParticles", e.target.value)}
            >
          ...
          {c.fxParticles && c.fxParticles !== "none" && (
            <>
              <SliderField
                label="Count"
                value={c.fxParticleCount ?? 25}
                onChange={(v) => set("fxParticleCount", v)}
                min={5}
                max={80}
              />
              <SliderField
                ...
                onChange={(v) => set("fxParticleSpeed", v)}
                min={0}
                max={100}
                suffix="%"
              />
              <SliderField
                ...
                onChange={(v) => set("fxParticleSize", v)}
                min={10}
                max={100}
                suffix="%"
              />
          ...
          <label className="nb-field">
            ...
            <select
              value={c.fxFog || "none"}
              onChange={(e) => set("fxFog", e.target.value)}
            >
              <option value="none">None</option>
          ...
          <label className="nb-field">
            <span>Type</span>
            <select
              value={c.fxGlimpse || "none"}
              onChange={(e) => set("fxGlimpse", e.target.value)}
            >
              <option value="none">None</option>
          ...
          {c.fxGlimpse && c.fxGlimpse !== "none" && (
            <SliderField
              ...
              onChange={(v) => set("fxGlimpseSpeed", v)}
              min={0}
              max={100}
              suffix="%"
            />

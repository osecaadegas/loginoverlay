...
export default function RtpStatsConfig({ config, onChange, allWidgets }) {
  ...
  const { set, setMulti } = makePerStyleSetters(
    ...
    c,
    currentStyle,
    RTP_STATS_STYLE_KEYS,
  );
  const [activeTab, setActiveTab] = useState("visibility");

  /* ─── Check if bonus hunt widget exists ─── */
  const bhWidget = (allWidgets || []).find(
    (w) => w.widget_type === "bonus_hunt",
  );
  ...
  const deletePreset = (name) =>
    set(
      "rtpPresets",
      (c.rtpPresets || []).filter((p) => p.name !== name),
    );
    ...
    <div className="bh-config">
      ...
      <div
        style={{
          padding: "8px 12px",
          borderRadius: 8,
          background: bhConfig.bonusOpening
            ? "rgba(34,197,94,0.15)"
            : "rgba(239,68,68,0.15)",
      ...
      {!bhWidget && (
        ...
        >
          ⚠️ No Bonus Hunt widget found. Add one first — the RTP bar reads slot
          data from it.
        </div>
      )}
      ...
      {activeTab === "visibility" && (
        <div className="nb-section">
          ...
          <label
            className="ov-chat-cfg-platform-header"
            style={{
              marginBottom: 10,
              padding: "8px 10px",
              borderRadius: 8,
          ...
          >
            <input
              type="checkbox"
              checked={!!c.previewMode}
              onChange={(e) => set("previewMode", e.target.checked)}
            />
            <span style={{ fontWeight: 600 }}>
              👁️ Always Show (Preview Mode)
            </span>
          </label>
          <p
            className="oc-config-hint"
            style={{ marginBottom: 12, marginTop: -4, fontSize: 11 }}
          >
            When ON, the bar always shows with demo data so you can position and
            style it. Turn OFF for live — bar will only appear during Bonus
            Opening.
          </p>
          ...
          >
            <input
              type="checkbox"
              checked={c.showSpinner !== false}
              onChange={(e) => set("showSpinner", e.target.checked)}
            />
          ...
          >
            <input
              ...
              onChange={(e) => set("showRtp", e.target.checked)}
            />
            <span>Show RTP</span>
          </label>
          ...
          >
            <input
              ...
              onChange={(e) => set("showPotential", e.target.checked)}
            />
          ...
          >
            <input
              ...
              onChange={(e) => set("showVolatility", e.target.checked)}
            />
          ...
          >
            <input
              ...
              onChange={(e) => set("showBestWin", e.target.checked)}
            />
      ...
      {activeTab === "presets" && (
        <div className="nb-section">
          ...
          <p className="oc-config-hint" style={{ marginBottom: 8 }}>
            Save and load presets for the RTP bar.
          </p>

...
export default function SlideshowFrameWidget({ config }) {
  const c = config || {};
  ...
  useEffect(() => {
    if (c.autoplay === false || mediaItems.length <= 1) return undefined;
  ...
  return (
    ...
    >
        ...
        {active ? (
          active.type === "video" ? (
            <video
              ...
              preload="auto"
              onEnded={() => {
                if (c.videoLoop === false && mediaItems.length > 1) {
                  setActiveIndex((index) => (index + 1) % mediaItems.length);
                }
              }}
            />
      ...
      {c.showCounter === true && mediaItems.length > 1 ? (

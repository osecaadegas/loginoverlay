...
export default function NavbarConfig({ config, onChange }) {
  ...
  const setSectionZone = (sectionId, newZone) => {
    const updated = sectionLayout.map((s) =>
      s.id === sectionId ? { ...s, zone: newZone } : s,
    );
    setLayout(updated);
  };
  ...
  const moveSectionUp = (sectionId) => {
    ...
    for (let i = idx - 1; i >= 0; i--) {
      if (sectionLayout[i].zone === item.zone) {
        prevIdx = i;
        break;
      }
    }
      ...
      {activeTab === "setup" && (
        <div className="nb-section">
          ...
          <label className="nb-toggle-row">
            <input
              type="checkbox"
              checked={!!c.showNowPlaying}
              onChange={(e) => set("showNowPlaying", e.target.checked)}
            />
          ...
          <label className="nb-toggle-row">
            <input
              ...
              onChange={(e) => set("showCrypto", e.target.checked)}
            />
          ...
          <label className="nb-toggle-row">
            <input
              ...
              onChange={(e) => set("showStartBalance", e.target.checked)}
            />
          ...
          <label className="nb-toggle-row">
            <input
              ...
              onChange={(e) => set("showCasino", e.target.checked)}
            />
                ...
                {[
                  ...
                  <label key={m.value} className="nb-radio">
                    <input
                      ...
                      value={m.value}
                      checked={
                        (c.cryptoDisplayMode || "horizontal") === m.value
                      }
                ...
                <div key={p.name} className="nb-preset-pill">
                  <div className="nb-preset-pill__info">
                    ...
                    <div className="nb-preset-pill__swatches">
                      {["accentColor", "bgColor", "textColor", "ctaColor"].map(
                        (k) =>
                          p.values[k] ? (
                            <span
                              key={k}

...
function BetterGiveawayRoulette({
  ...
  const winnerParticipant =
    crowd.find((participant) => participant.name.toLowerCase() === winnerKey) ||
    crowd[0];
...
const BETTER_BETS_THEME_VARS = {
  neon: {
    ...
    "--frame-bg": "linear-gradient(180deg,#0c1c40 0%,#0a1734 55%,#081228 100%)",
    "--frame-shadow":
      "0 0 0 1px rgba(0,0,0,0.55),inset 0 1px 0 color-mix(in srgb,#9dbdf2 12%,transparent)",
    "--bracket-color": "#2f63c9",
    "--bracket-filter":
      "drop-shadow(0 0 3px color-mix(in srgb,#45c8ff 40%,transparent))",
...
const BETTER_HUNT_REQUEST_ROWS = {
  compact: 3,
  image: 2,
  names: 4,
};
...
function BetterStyleSheet() {
  return (
    <style>{`
      ...
      .ov-chat-giveaway-canvas .better-giveaway-stage{padding:0}
      .ov-chat-giveaway-canvas .better-giveaway-widget{width:100%}
      ...
      .better-giveaway-widget[data-surface="matte"]{background:linear-gradient(180deg,var(--w-panel-hi,#0c1c40) 0%,var(--w-bg,#0a1734) 55%,var(--w-panel-lo,#081228) 100%);box-shadow:0 0 0 1px rgba(0,0,0,.55),inset 0 1px 0 color-mix(in srgb,var(--w-steel-hi,#9dbdf2) 12%,transparent)}
      .better-giveaway-widget::before{position:absolute;inset:var(--w-inner-inset,5px);z-index:-1;border:1px solid color-mix(in srgb,var(--w-line,#2f63c9) 55%,transparent);border-radius:max(0px,calc(var(--w-radius,12px) - var(--w-inner-inset,5px)));opacity:var(--w-inner-op,1);pointer-events:none;content:"";box-shadow:inset 0 0 calc(13px * var(--IG)) color-mix(in srgb,var(--w-accent,#45c8ff) 12%,transparent);transition:inset 200ms ease,opacity 240ms ease,border-radius 320ms ease}
      ...
      .better-gw-metric-panel{position:relative;display:flex;min-width:0;min-height:clamp(34px,22cqh,50px);flex-direction:column;align-items:center;justify-content:center;overflow:hidden;padding:6px 10px;border:1px solid color-mix(in srgb,var(--w-line,#2f63c9) 55%,transparent);border-radius:var(--w-tile-radius,10px);background:linear-gradient(180deg,var(--w-card-hi,#0d2049),var(--w-card-lo,#0a1836));box-shadow:inset 0 1px 0 color-mix(in srgb,var(--w-steel-hi,#9dbdf2) 10%,transparent),0 2px 8px rgba(0,0,0,.5);transition:border-radius 320ms ease,background 480ms ease}
      ...
      .better-giveaway-widget[data-surface="matte"] .better-gw-metric-panel{background:linear-gradient(180deg,var(--w-card-hi,#0d2049),var(--w-card-lo,#0a1836));box-shadow:inset 0 1px 0 color-mix(in srgb,var(--w-steel-hi,#9dbdf2) 10%,transparent),0 2px 8px rgba(0,0,0,.5)}
      ...
      @keyframes better-hunt-request-ring-reveal{0%{opacity:.82}12%,100%{opacity:1}}
      @keyframes better-hunt-request-charge{from{transform:translateX(-140%) skewX(-18deg);opacity:0}35%{opacity:1}to{transform:translateX(140%) skewX(-18deg);opacity:0}}
      @keyframes better-hunt-request-landing{0%{transform:scale(1.28);opacity:0}22%{opacity:1}48%{transform:scale(.94)}72%{transform:scale(1.04)}to{transform:scale(1);opacity:0}}
      ...
      @keyframes better-hunt-win-rays{0%{transform:translate(-50%,-50%) rotate(0deg) scale(.94);filter:hue-rotate(0deg) saturate(1.15);opacity:.42}50%{transform:translate(-50%,-50%) rotate(180deg) scale(1.06);filter:hue-rotate(180deg) saturate(1.45);opacity:.68}100%{transform:translate(-50%,-50%) rotate(360deg) scale(.94);filter:hue-rotate(360deg) saturate(1.15);opacity:.42}}
      ...
      .better-hunt-mainstream{max-width:min(100%,var(--bh-panel-width,372px));height:var(--bh-panel-height,auto);display:grid;grid-template-rows:auto auto auto auto auto minmax(0,1fr) auto;gap:0;padding:0}
      .better-hunt-mainstream--no-requests{grid-template-rows:auto auto auto auto minmax(0,1fr) auto}
      ...
      .better-hunt-main-avatar-fallback{width:var(--bh-avatar);height:var(--bh-avatar);display:grid;place-items:center;border-radius:50%;background:radial-gradient(circle at 30% 20%,color-mix(in srgb,var(--bh-ice) 45%,transparent),var(--bh-inset));color:#fff;font-size:.72em;font-weight:950}
      .better-hunt-main-live-dot{position:absolute;right:0;top:0;width:9px;height:9px;border:2px solid var(--bh-panel-lo);border-radius:50%;background:var(--bh-tangerine);box-shadow:0 0 9px var(--bh-tangerine)}
      .better-hunt-main-title-wrap{min-width:0;flex:1}
      .better-hunt-main-name{overflow:hidden;margin:0;color:#fff;font-size:1.28em;font-style:italic;font-weight:950;letter-spacing:.16em;line-height:1;text-overflow:ellipsis;text-transform:uppercase;white-space:nowrap;text-shadow:0 1px 4px rgba(0,0,0,.65)}
      ...
      .better-hunt-main-progress-count{color:var(--bh-steel-hi);font-size:.92em;font-weight:950;letter-spacing:.08em;line-height:1;white-space:nowrap}.better-hunt-main-progress-count strong{color:var(--bh-ice);text-shadow:0 0 10px color-mix(in srgb,var(--bh-ice) 55%,transparent)}
      .better-hunt-main-active{display:flex;min-height:142px;overflow:hidden;margin:10px 12px 0;border:1px solid rgba(255,255,255,.12);border-radius:12px;box-shadow:0 6px 22px rgba(0,0,0,.6);animation:better-hunt-stats-in calc(.6s / var(--anim-speed,1)) cubic-bezier(.22,.9,.3,1) both}
      .better-hunt-main-active--normal{background:linear-gradient(90deg,#12b76a,#0a7a48)}.better-hunt-main-active--super{border-color:#ffd23d;background:linear-gradient(90deg,#e8b923,#a8760e);animation:better-hunt-stats-in calc(.6s / var(--anim-speed,1)) cubic-bezier(.22,.9,.3,1) both,better-hunt-gold calc(2.4s / var(--anim-speed,1)) ease-in-out infinite}.better-hunt-main-active--extreme{border-color:rgba(255,84,112,.72);border-style:dashed;background:linear-gradient(90deg,#e8452a,#8f2110)}
      ...
      .better-hunt-hstrip-active--super{box-shadow:inset 0 0 0 2px #ffd23d,inset 0 0 24px rgba(255,210,61,.28)}.better-hunt-hstrip-active--extreme{box-shadow:inset 0 0 0 2px #ff5470,inset 0 0 26px rgba(255,84,112,.3)}
      ...
      .better-hunt-main-carousel .better-hunt-stats-content{height:auto;min-height:210px;gap:12px}
      .better-hunt-main-carousel .better-hunt-stats-title>div{min-width:0;flex:1}
      .better-hunt-main-carousel .better-hunt-tier{min-width:0;max-width:45%;overflow-wrap:anywhere}
      .better-hunt-main-carousel .better-hunt-stat-strip .better-hunt-stat-label{display:block;overflow-wrap:anywhere;letter-spacing:0}
      ...
      .better-hunt-autoscroll-copy{position:absolute;inset:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-template-rows:auto minmax(0,1fr) auto;align-items:start;gap:4px;min-width:0;padding:0;line-height:1.1}
      .better-hunt-autoscroll-stat{display:grid;min-width:0;width:max-content;max-width:100%;padding:1px 2px;border-radius:0;background:color-mix(in srgb,var(--bh-inset) 88%,transparent);text-shadow:0 1px 2px #000;letter-spacing:0;font-variant-numeric:tabular-nums}
      ...
      .better-hunt-request-transfer{contain:layout paint style;filter:brightness(1.12) saturate(1.14)}.better-hunt-request-transfer>.better-hunt-request-transfer-card{animation:better-hunt-request-source-fade calc(1.35s / var(--anim-speed,1)) linear both}.better-hunt-request-transfer-ring{position:absolute;inset:0;z-index:2;overflow:hidden;border:2px solid var(--bh-line-hi);border-radius:10px;background:var(--bh-inset);box-shadow:0 0 28px color-mix(in srgb,var(--bh-ice) 68%,transparent),0 10px 24px rgba(0,0,0,.72),inset 0 0 0 1px rgba(255,255,255,.18);opacity:0;animation:better-hunt-request-ring-reveal calc(1.35s / var(--anim-speed,1)) linear both}.better-hunt-request-transfer-ring img,.better-hunt-request-transfer-ring>.better-hunt-thumb{width:100%;height:100%;display:block;border-radius:0;object-fit:cover;opacity:1}.better-hunt-request-transfer-ring::after{content:"";position:absolute;inset:0 0 auto;height:34%;background:linear-gradient(180deg,rgba(255,255,255,.22),transparent)}.better-hunt-request-shard{contain:strict;backface-visibility:hidden;transform:translateZ(0)}.better-hunt-request-shard>.better-hunt-request-shard-surface{contain:layout paint style}
      ...
      .better-hunt-result--compact{--bh-result-art-col:2;--bh-result-copy-col:1;grid-template-columns:minmax(0,1fr) minmax(54px,42%);grid-template-rows:auto auto minmax(0,1fr);gap:4px;padding:4px}
      .better-hunt-result--compact.better-hunt-result--worst{--bh-result-art-col:1;--bh-result-copy-col:2;grid-template-columns:minmax(54px,42%) minmax(0,1fr)}
      .better-hunt-result--compact .better-hunt-result-head{grid-column:1/-1;grid-row:1;grid-template-columns:auto minmax(0,1fr);align-items:center;gap:4px}
      .better-hunt-result--compact .better-hunt-result-head em{font-size:.6em}
      .better-hunt-result--compact .better-hunt-result-slot{font-size:.74em}
      .better-hunt-result--compact .better-hunt-result-body{display:contents}
      .better-hunt-result--compact .better-hunt-result-art{grid-column:var(--bh-result-art-col);grid-row:2/4;height:100%;min-height:88px}
      .better-hunt-result--compact .better-hunt-result-payout{grid-column:var(--bh-result-copy-col);grid-row:2;gap:2px}
      .better-hunt-result--compact .better-hunt-result-payout>span,.better-hunt-result--compact .better-hunt-result-row>span{font-size:.56em}
      .better-hunt-result--compact .better-hunt-result-payout>strong{font-size:.94em}
      .better-hunt-result--compact .better-hunt-result-stats{grid-column:var(--bh-result-copy-col);grid-row:3;grid-template-columns:minmax(0,1fr);align-content:start;gap:4px;border:0;padding-top:0}
      .better-hunt-result--compact .better-hunt-result-row{gap:1px}
      .better-hunt-result--compact .better-hunt-result-row>strong{font-size:.84em}
      .better-hunt-result--compact.better-hunt-result--best .better-hunt-result-payout,.better-hunt-result--compact.better-hunt-result--best .better-hunt-result-stats{text-align:right}
      ...
      .better-hunt-requests--carousel .better-hunt-request-stage{height:166px;min-height:166px}
      ...
      .better-hunt-root[data-anim="off"]::before,.better-hunt-root[data-anim="off"]::after,.better-hunt-root[data-anim="off"] .better-hunt-list-inner,.better-hunt-root[data-anim="off"] .better-hunt-request-track,.better-hunt-root[data-anim="off"] .better-hunt-request-live,.better-hunt-root[data-anim="off"] .better-hunt-request-fx,.better-hunt-root[data-anim="off"] .better-hunt-lane-track,.better-hunt-root[data-anim="off"] .better-hunt-card--center,.better-hunt-root[data-anim="off"] .better-hunt-card--super,.better-hunt-root[data-anim="off"] .better-hunt-card--extreme,.better-hunt-root[data-anim="off"] .better-hunt-stats-image img,.better-hunt-root[data-anim="off"] .better-hunt-image-stats-panel--super,.better-hunt-root[data-anim="off"] .better-hunt-image-stats-art img,.better-hunt-root[data-anim="off"] .better-hunt-hstrip-slot-stats--super,.better-hunt-root[data-anim="off"] .better-hunt-hstrip-slot-art img,.better-hunt-root[data-anim="off"] .better-hunt-main-active,.better-hunt-root[data-anim="off"] .better-hunt-main-active-art img,.better-hunt-root[data-anim="off"] .better-hunt-main-active-shroud,.better-hunt-root[data-anim="off"] .better-hunt-panel--shake,.better-hunt-root[data-anim="off"] .better-hunt-win,.better-hunt-root[data-anim="off"] .better-hunt-win *{animation:none!important}
...
export function BetterBonusHuntStyle({
  ...
  const configuredDrawerMode = c.drawerMode === "expand" ? "expand" : "contain";
  ...
  const requestActiveIndex = useBetterHuntCarousel(
    requestRows.length,
    Number(c.carouselMs) || 3200,
    c.animations !== false && requestView === "carousel",
  );
  ...
  const measureRequestRect = (rootNode, node) => {
    ...
    return {
      left: (nodeRect.left - rootRect.left) / scaleX,
      top: (nodeRect.top - rootRect.top) / scaleY,
  ...
  useEffect(() => {
    const fresh = safeArray(c.requestActions).filter((action) => {
      if (!action?.id || !action?.action) return false;
      const key = `${action.id}:${action.action}:${action.action_at || action.updated_at || ""}`;
      if (seenRequestActionsRef.current.has(key)) return false;
  ...
  useLayoutEffect(() => {
    ...
    const targetNode =
      ...
      rootRef.current.querySelector(".better-hunt-image-stats-art") ||
      rootRef.current.querySelector(".better-hunt-carousel");
    const targetRect = measureRequestRect(rootNode, targetNode);
    const target = {
      left: targetRect?.left || rootNode.clientWidth / 2 - 48,
      top: targetRect?.top || 110,
      width: targetRect?.width || 96,
      height: targetRect?.height || 142,
    };
    setRequestActionVisual({
      action,
      source,
      target,
    });
  ...
  useEffect(() => {
    if (!requestActionVisual) return undefined;
    const duration =
      requestActionVisual.action.action === "accepted" ? 1550 : 980;
  ...
  const renderRequestAction = () => {
    ...
    if (action.action === "accepted") {
      ...
      const growScale = Math.max(1, Math.min(2.1, 128 / source.width));
      const endScaleX = target.width / source.width;
      const endScaleY = target.height / source.height;
      return (
        <div className="better-hunt-request-fx" aria-hidden="true">
          <div
            className="better-hunt-request-transfer"
            style={{
              left: source.left,
              top: source.top,
              width: source.width,
              height: source.height,
              "--bh-transfer-x": `${targetCenterX - sourceCenterX}px`,
              "--bh-transfer-y": `${targetCenterY - sourceCenterY}px`,
              "--bh-transfer-grow": growScale,
          ...
          >
            ...
            <span className="better-hunt-request-transfer-ring">
              <BetterHuntThumb
                bonus={{
                  ...action,
                  image: requestImage(action),
                  slotName: requestSlot(action),
                }}
                size={112}
    ...
    const columns = Math.max(5, Math.min(8, Math.round(source.width / 44)));
    const shardRows = Math.max(2, Math.min(3, Math.round(source.height / 34)));
    const shardWidth = source.width / columns;
    const shardHeight = source.height / shardRows;
    return (
      <div className="better-hunt-request-fx" aria-hidden="true">
        <div
          className="better-hunt-request-flash"
          style={{
            left: source.left,
            top: source.top,
            width: source.width,
        ...
        />
        {Array.from({ length: columns * shardRows }, (_, index) => {
          ...
          return (
            <i
              ...
              style={{
                ...
                "--bh-shard-x": `${(column / (columns - 1) - 0.5) * 190}px`,
                "--bh-shard-y": `${65 + row * 28 + (index % 3) * 18}px`,
  ...
  const renderRequests = () => {
    ...
    const requestsScroll =
      ...
      c.animations !== false;
    const renderRequest = (request, index, copy = 0) => {
      const key =
        request?.id ||
      ...
      >
        ...
        {requestRows.length ? (
          requestView === "carousel" ? (
            <div className="better-hunt-request-stage">
              <div className="better-hunt-request-ring">
                {requestRows.map((request, index) => {
                  let distance = index - requestActiveIndex;
                  if (distance > requestRows.length / 2)
                    distance -= requestRows.length;
                  if (distance < -requestRows.length / 2)
                    distance += requestRows.length;
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        filter: center
                          ? "none"
                          : "brightness(.52) saturate(.7)",
                        transform: `translate3d(-50%, -50%, 0) translate3d(${distance * 88}px, 0, ${-abs * 100}px) rotateY(${distance * -32}deg) scale(${center ? 1.1 : 0.85})`,
                      }}
                    >
                      <SlotImage
                      ...
                      />
                      <span className="better-hunt-request-card-copy">
                        @{requestName(request)}
                      </span>
                    </div>
                  );
            ...
            >
              <div className="better-hunt-request-track">
                <div className="better-hunt-request-group">
                  {requestRows.map((request, index) =>
                    renderRequest(request, index),
                  )}
                </div>
                {requestsScroll ? (
                  <div className="better-hunt-request-group" aria-hidden="true">
                    {requestRows.map((request, index) =>
                      renderRequest(request, index, 1),
                    )}
                  </div>
                ) : null}
              </div>
  ...
  const renderProgress = () => (
    <div className="better-hunt-progress">
      ...
      {orientation !== "mainstream" && <div className="better-hunt-counts" aria-label="Bonus tiers">
        <span
      ...
      >
        {rows.length ? (
          <div className="better-hunt-list-inner">
            <div className="better-hunt-list-group">
              {listRows.map((bonus, index) =>
                renderListRow(bonus, index, listMode, index),
              )}
            </div>
            {rows.length > visibleRows && c.animations !== false ? (
              <div className="better-hunt-list-group" aria-hidden="true">
                {listRows.map((bonus, index) =>
                  renderListRow(bonus, index, listMode, `copy-${index}`),
                )}
              </div>
            ) : null}
          </div>
  ...
  const renderResultCard = (label, bonus, variant) => {
    ...
    return (
      <div
        className={`better-hunt-result better-hunt-result--${variant}${orientation !== "horizontal" ? " better-hunt-result--compact" : ""}`}
      ...
      >
        ...
        <div className="better-hunt-main-active-stats">
          {activeRows.map(([label, value, Icon]) => (
            <div key={label} className="better-hunt-main-active-row">
              <span {...attrs("bonus_hunt", c, "statLabel")}>
  ...
  const renderHorizontalStrip = () => {
    ...
    const currentSlotStats = current
      ? [
          [
            "Bet",
            bonusBet(current) > 0 ? formatMoney(bonusBet(current), money) : "-",
          ...
          [
            "Volatility",
            <BetterHuntVolatilityBars
              key="horizontal-volatility"
              value={bonusVolatility(current)}
            />,
          ],
          ["Max Win", bonusMaxWin(current)],
          ["Best", bestWinValue],
    ...
    const currentTier = bonusTier(current);
    ...
    return (
      ...
      >
        <div className="better-hunt-hstrip-list">
          ...
          {current ? (
            <div
              className={`better-hunt-hstrip-slot-stats better-hunt-hstrip-slot-stats--${currentTier}`}
              {...attrs("bonus_hunt", c, "slotCarouselContainer")}
            >
              <div className="better-hunt-hstrip-slot-art">
                <SlotImage
                  src={bonusImage(current)}
                  alt={bonusSlotName(current, activeIndex)}
                  {...attrs("bonus_hunt", c, "slotImage")}
              ...
              <div className="better-hunt-hstrip-slot-rows">
                {currentSlotStats.map(([label, value]) => (
                  <div className="better-hunt-hstrip-slot-row" key={label}>
                    <span
                      className="better-hunt-stat-label"
                      {...attrs("bonus_hunt", c, "statLabel")}
                    >
        ...
        <aside
          className="better-hunt-hstrip-requests"
          aria-hidden={c.showRequests === false}
        >
          {renderRequests()}
        </aside>
...
export function BetterGiveawayStyle({ config }) {
  ...
  const vars = {
    ...
    "--w-panel-lo": c.panelLo || "#081228",
    "--w-card-hi": c.cardHi || "#0d2049",
    "--w-card-lo": c.cardLo || "#0a1836",
    "--w-line": c.lineColor || "#2f63c9",
    "--w-steel-hi": c.secondaryTextColor || "#9dbdf2",
    "--w-accent": c.accentColor || "#45c8ff",
    "--w-accent-2": c.deepAccentColor || "#1e5ad6",

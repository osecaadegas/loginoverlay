...
export const RECOMMENDED_TOOLS = new Set([
  "bonus_hunt",
  "navbar",
  "background",
]);

const CHAT_TOOL_TYPES = new Set([
  "slot_requests",
  "chat",
  "giveaway",
  "bets",
...
export function getToolSetupIssues(type, widget, integrations = {}) {
  ...
  if (!widget) {
    issues.push({
      kind: "setup",
      label: "Tool is not installed",
      to: getToolRoute(type),
    });
    return issues;
  }
  ...
  switch (type) {
    ...
    case "bonus_buys":
      if (!Array.isArray(config.bonuses) || config.bonuses.length === 0) {
        issues.push({
          kind: "setup",
          label: "Add bonus-buy entries",
          to: getToolRoute(type),
        });
      }
      break;
    case "tournament": {
      const matches = Array.isArray(config.setupMatches)
        ? config.setupMatches
        : [];
      const hasPlayers = matches.some((match) =>
        hasAnyText(match?.player1, match?.player2),
      );
      if (!config.data && !hasPlayers) {
        issues.push({
          kind: "setup",
          label: "Create tournament players",
          to: getToolRoute(type),
        });
      }
      break;
    ...
    case "navbar":
      if (
        !hasAnyText(config.streamerName, config.motto, config.twitchUsername)
      ) {
        issues.push({
          kind: "setup",
          label: "Add streamer branding",
          to: getToolRoute(type),
        });
      }
      if (
        config.showNowPlaying &&
        config.musicSource === "spotify" &&
        !integrations.spotifyConnected
      ) {
        issues.push({
          kind: "connection",
          label: "Spotify is not connected",
    ...
    case "chat":
      if (
        !getTwitchReady(config, integrations) &&
        !getKickReady(config) &&
        !getYoutubeReady(config)
      ) {
        issues.push({
          kind: "connection",
    ...
    case "slideshow_frame":
      if (
        ...
        issues.push({
          kind: "setup",
          label: "Add image or video links",
          to: "/editor",
        });
      }
      break;
    ...
    case "bets":
      if (!Array.isArray(config.options) || config.options.length < 2) {
        issues.push({
          kind: "setup",
          label: "Add at least two bet options",
          to: getToolRoute(type),
        });
      }
    ...
    case "spotify_now_playing":
      if (
        !integrations.spotifyConnected &&
        !hasAnyText(config.spotify_access_token, config.manualTrack)
      ) {
        issues.push({
          kind: "connection",
          label: "Spotify is not connected",
  ...
  if (
    CHAT_TOOL_TYPES.has(type) &&
    config.twitchEnabled &&
    !getTwitchReady(config, integrations)
  ) {
    issues.push({
      kind: "connection",
...
export function resolveToolStatus({
  ...
  if (premiumLocked) {
    return {
      ...
      issues: [
        { kind: "premium", label: "Premium access required", to: "/premium" },
      ],
    };
  }
...
export function summarizeOverlayTools({
  toolTypes,
  widgets,
  integrations = {},
  premiumLocked = false,
  ...
  const installed = toolTypes
    .map((type) => ({
      type,
      widget: widgets.find((item) => item.widget_type === type),
    }))
  ...
  const ready = statuses.filter(
    (item) => item.status.type === TOOL_STATUS.READY,
  );
  const issues = statuses.flatMap((item) =>
    item.status.issues.map((issue) => ({ ...issue, toolType: item.type })),
  );
  const uniqueIssues = [];
  const seen = new Set();

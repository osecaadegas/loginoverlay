  ...
  return (
    ...
    >
      ...
      <div className="pl-card__preview">
        ...
        {type === "shared" && (
          <span className="pl-card__badge pl-card__badge--shared">
            🌐 Shared
          </span>
        )}
        {type === "personal" && (
          <span className="pl-card__badge pl-card__badge--personal">
            👤 Mine
          </span>
        )}
        {preset.fullBackup && (
          <span className="pl-card__badge pl-card__badge--backup">
            Full backup
          </span>
        )}
      </div>
      ...
      <div className="pl-card__body">
        ...
        <div className="pl-card__meta">
          <span className="pl-card__widget-count">
            {widgetTypes.slice(0, 6).join(" ")} · {widgetCount} widget
            {widgetCount !== 1 ? "s" : ""} · {backupLabel}
          </span>
        </div>
        ...
        <div className="pl-card__actions">
          <button
            className="pl-card__btn pl-card__btn--load"
          ...
          {type === "personal" && isAdmin && onShare && (
            <button
              className="pl-card__btn pl-card__btn--share"
          ...
          {type === "shared" && isAdmin && onUnshare && (
            <button
              className="pl-card__btn pl-card__btn--unshare"
              onClick={() => onUnshare(preset.id)}
          ...
          {((type === "personal" && onDelete) ||
            (type === "shared" && isAdmin && onDelete)) && (
            <button
              className="pl-card__btn pl-card__btn--delete"
              onClick={() =>
                onDelete(type === "shared" ? preset.id : preset.name)
              }
              title="Delete"
            >
              🗑️
...
export default function PresetLibrary({
  ...
  const allPresets = useMemo(() => {
    const personal = (globalPresets || []).map((p) => ({
      ...p,
      _type: "personal",
    }));
    const shared = (sharedPresets || []).map((p) => ({
      ...p,
      _type: "shared",
    }));
  ...
  const filtered = useMemo(() => {
    ...
    // Filter by type
    if (filter === "personal")
      list = list.filter((p) => p._type === "personal");
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sortBy === "widgets") {
        const countA = (a.snapshot || []).filter(
          (s) => s.is_visible !== false,
        ).length;
        const countB = (b.snapshot || []).filter(
          (s) => s.is_visible !== false,
        ).length;
  ...
  const handleDeleteShared = useCallback(
    (id) => {
  ...
  const totalVisibleWidgets = allPresets.reduce(
    (sum, preset) =>
      sum +
      (preset.snapshot || []).filter((s) => s.is_visible !== false).length,
    0,
  );
  const largestPreset = allPresets.reduce((largest, preset) => {
    const count = (preset.snapshot || []).filter(
      (s) => s.is_visible !== false,
    ).length;
  ...
  const sharedRatio =
    totalCount > 0 ? Math.round((sharedCount / totalCount) * 100) : 0;
  const pageNote =
    totalCount > 0
      ...
      <div className="pl-page-shell">
        <div className="pl-hero">
          ...
          <div className="pl-hero__metrics">
            <div className="pl-hero__metric-card">
              ...
              <span className="pl-hero__metric-meta">
                {personalCount} personal, {sharedCount} shared
              </span>
            </div>
            <div className="pl-hero__metric-card">
              ...
              <span className="pl-hero__metric-meta">
                {search ? `Matching "${search}"` : `Filter: ${filter}`}
              </span>
            </div>
            <div className="pl-hero__metric-card">
              ...
              <span className="pl-hero__metric-meta">
                Shared availability across the vault
              </span>
            </div>
            <div className="pl-hero__metric-card">
              <span className="pl-hero__metric-label">Largest Layout</span>
              <strong className="pl-hero__metric-value pl-hero__metric-value--name">
                {largestPreset?.name || "No presets yet"}
              </strong>
              <span className="pl-hero__metric-meta">
                {largestPreset
                  ? `${largestPreset.count} visible widgets`
                  : "Save a preset to start ranking layouts"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="pl-section-heading">
          <div>
            ...
            <h3 className="pl-section-heading__title">
              Capture the current overlay into your widget library
            </h3>
          </div>
          <span className="pl-section-heading__pill">
            {totalVisibleWidgets} widgets across all presets
          </span>
        </div>
        ...
        <div className="pl-save-card">
          ...
          <p className="pl-save-card__note">
            Save the current widget setup, positions, sizes, visibility, V2
            appearance edits, and non-secret configuration so you can restore a
            working build after major edits.
          </p>
        </div>
        ...
        <div className="pl-section-heading pl-section-heading--compact">
          <div>
            ...
            <h3 className="pl-section-heading__title">
              Search the gallery, switch catalogs, and reorder what matters
            </h3>
          </div>
          <span className="pl-section-heading__pill">
            {filteredCount} shown
        ...
        <div className="pl-toolbar-card">
          ...
          <p className="pl-toolbar-card__note">
            Every preset card below keeps its live thumbnail, load flow, share
            controls, and delete controls exactly as before.
          </p>
        </div>
          ...
          <div className="pl-empty" data-tour="presets-shared">
            ...
            <p className="pl-empty__text">
              {search
                ...
                : "Save your first preset from the sidebar to get started."}
            </p>
          </div>
        ) : (
          <>
            <div className="pl-section-heading pl-section-heading--compact">
              <div>
                <span className="pl-section-heading__eyebrow">
                  Backup Gallery
                </span>
                <h3 className="pl-section-heading__title">
                  Browse saved builds and restore the right version
                </h3>
              </div>

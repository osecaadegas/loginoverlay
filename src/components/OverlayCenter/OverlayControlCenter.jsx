...
const INTEGRATIONS = [
  ...
  },
  {
    id: "spotify",
    name: "Spotify",
    requiredFor: ["navbar", "spotify_now_playing"],
    detail: "Optional music data for Navbar and Spotify widgets.",
  },
  {
    id: "slots",
    name: "Slot Database",
    requiredFor: ["bonus_hunt", "current_slot", "rtp_stats", "slot_requests"],
    detail: "Used for slot images, RTP, provider and game metadata.",
  },
];
...
function getOverlayUrl(instance, { preview = false, widgetId = null } = {}) {
  ...
  const url = new URL(
    `/overlay/${instance.overlay_token}`,
...
function defaultSetupState(widgets = [], theme = null, instance = null) {
  ...
  return {
    ...
    selectedTools: completed
      ? widgets.map((widget) => widget.widget_type)
      : ["navbar", "background", "bonus_hunt"],
    details: {
      ...
      twitchChannel:
        chatConfig.twitchChannel ||
        slotRequestsConfig.twitchChannel ||
        betsConfig.twitchChannel ||
        "",
      commandPrefix: "!",
      slotRequestCommand: normalizeCommandName(
        slotRequestsConfig.commandTrigger,
        "sr",
      ),
      betCommand: normalizeCommandName(betsConfig.chatCommand, "bet"),
      giveawayKeyword: normalizeCommandName(
        configFor("giveaway").keyword,
        "join",
      ),
      currencyCode: normalizeCurrencyCode(
        bonusHuntConfig.currencyCode ||
          currentSlotConfig.currencyCode ||
          navbarConfig.currencyCode ||
          bonusHuntConfig.currency ||
          currentSlotConfig.currency ||
          navbarConfig.balanceCurrency ||
          "EUR",
      ),
      ...
      pointSource:
        slotRequestsConfig.srSeEnabled || betsConfig.betSeEnabled
          ? "streamelements"
          : "internal",
      ...
      pointBalanceBehavior:
        slotRequestsConfig.pointBalanceBehavior || "charge_immediately",
      insufficientPointsBehavior:
        slotRequestsConfig.insufficientPointsBehavior || "reject",
      refundBehavior:
        slotRequestsConfig.refundBehavior || "refund_on_cancel_or_reject",
      musicMode:
        navbarConfig.musicSource === "spotify"
          ? "spotify"
          : navbarConfig.musicSource === "disabled"
            ? "disabled"
            : "manual",
      spotifyMode:
        navbarConfig.musicSource === "spotify" ? "spotify" : "manual",
      manualArtist:
        navbarConfig.manualArtist ||
        configFor("spotify_now_playing").manualArtist ||
        "",
      manualTrack:
        navbarConfig.manualTrack ||
        configFor("spotify_now_playing").manualTrack ||
        "",
      manualAlbum:
        navbarConfig.manualAlbum ||
        configFor("spotify_now_playing").manualAlbum ||
        "",
      manualCoverUrl:
        navbarConfig.manualCoverUrl ||
        configFor("spotify_now_playing").manualAlbumArt ||
        "",
      manualMusicLink: navbarConfig.manualMusicLink || "",
      musicFallbackMessage:
        navbarConfig.musicFallbackMessage || "No track playing",
      ...
      defaultRtpHandling:
        currentSlotConfig.defaultRtpHandling || "show_unknown",
      defaultVolatilityHandling:
        currentSlotConfig.defaultVolatilityHandling || "show_unknown",
      defaultProviderLabel:
        currentSlotConfig.defaultProviderLabel || "Unknown provider",
      missingSlotImageBehavior:
        currentSlotConfig.missingSlotImageBehavior || "use_default_image",
    },
    validationErrors: [],
  };
}
...
function mergeSetupState(raw, widgets, theme, instance) {
  if (raw?.status) {
    return {
      ...
      selectedTools:
        raw.selectedTools ||
        defaultSetupState(widgets, theme, instance).selectedTools,
...
function serviceSetupPatch(type, details = {}, integrations = {}) {
  ...
  const twitchChannel = normalizeTwitchChannel(
    normalized.twitchChannel || integrations.twitchChannel,
  );
  ...
  switch (type) {
    ...
    case "giveaway":
      return {
        twitchEnabled: Boolean(twitchChannel),
        twitchChannel,
        commandPrefix,
        keyword: normalizeCommand(
          normalized.giveawayKeyword,
          "join",
          commandPrefix,
        ),
    ...
    case "navbar":
      return {
        ...
        showNowPlaying:
          ...
          (musicSource === "manual" &&
            Boolean(
              manualArtist || manualTrack || normalized.musicFallbackMessage,
            )),
      };
    case "spotify_now_playing":
      return {
        ...
        manualTrack,
        manualAlbum: normalized.manualAlbum,
        manualAlbumArt: normalized.manualCoverUrl,
        musicFallbackMessage: normalized.musicFallbackMessage,
        hideMusicWhenEmpty: normalized.hideMusicWhenEmpty,
      };
    case "bonus_hunt":
...
function validateOverlay({ instance, widgets, setup, integrations = {} }) {
  ...
  for (const type of selected) {
    if (!widgets.some((widget) => widget.widget_type === type)) {
      errors.push(
        `${FEATURE_COPY[type]?.title || type} has not been added yet.`,
      );
    }
  }
  const needsChatChannel = selected.some((type) =>
    ["chat", "giveaway", "slot_requests", "bets"].includes(type),
  );
  if (
    needsChatChannel &&
    !normalizeTwitchChannel(details.twitchChannel || integrations.twitchChannel)
  ) {
    errors.push(
      "Add your Twitch channel so chat, requests, giveaways and bets can listen to the right stream.",
    );
  }
  const needsStreamElements =
    (selected.includes("slot_requests") && details.requestsUsePoints) ||
    (selected.includes("bets") && details.betsUsePoints !== false);
  if (needsStreamElements && !integrations.streamelementsConnected) {
    errors.push(
      "Connect StreamElements in Integrations before using point-paid requests or bets.",
    );
  }
  const needsSpotify =
    selected.includes("spotify_now_playing") ||
    (selected.includes("navbar") && details.spotifyMode === "spotify");
  if (
    needsSpotify &&
    !integrations.spotifyConnected &&
    !(details.manualArtist && details.manualTrack)
  ) {
    errors.push(
      "Connect Spotify or add a manual artist and track fallback for music widgets.",
    );
  }
  const needsCurrency = selected.some((type) =>
    [
      "bonus_hunt",
      "current_slot",
      "rtp_stats",
  ...
  if (needsCurrency && !details.currency)
    errors.push("Choose a currency symbol for money and slot widgets.");
  return errors;
}
...
function OverlayTopNavigation({ active, setupComplete }) {
  const navItems = [
    ...
    { id: "editor", label: "Editor", to: "/editor", icon: Brush },
  ];
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.id}
              to={item.to}
              className={`oc2-nav-link${active === item.id ? " oc2-nav-link--active" : ""}`}
            >
              <Icon size={16} />
    ...
    >
      <header className="oc2-tool-card__header">
        ...
        <div className="oc2-tool-card__title">
          ...
          {RECOMMENDED_TOOLS.has(type) && (
            <span className="oc2-pill oc2-pill--gold">Recommended</span>
          )}
        </div>
        {installed && (
          <label
            className="oc2-tool-toggle"
      ...
      <p className="oc2-tool-card__description">
        {copy.description ||
          def.description ||
          "Configure this overlay feature."}
      </p>
      ...
      <footer
        className="oc2-tool-card__actions"
        onClick={(event) => event.stopPropagation()}
      >
        <button
          type="button"
        ...
        {installed && (
          <details className="oc2-card-menu">
            ...
            <div className="oc2-card-menu__panel">
              <button type="button" onClick={() => onOpen(type)}>
                Open settings
              </button>
              ...
              <button
                type="button"
                className="oc2-card-menu__danger"
...
function ToolSection({ title, subtitle, tools, emptyText, children, tourId }) {
  return (
    <section className="oc2-tool-section" data-tour={tourId}>
      ...
      {tools.length > 0 ? (
        <div className="oc2-tool-grid">{tools}</div>
      ) : (
...
function PreviewToolRailCard({ tool, side, onOpen }) {
  ...
  return (
    <article className={`oc2-preview-tool-card oc2-preview-tool-card--${side}`}>
      ...
      <button
        type="button"
        className="oc2-preview-tool-card__action"
        onClick={() => onOpen(type)}
      >
        Configure
      </button>
...
function ToolsLivePreviewDock({
  ...
  return (
    <section
      className={`oc2-tools-preview-dock${expanded ? " oc2-tools-preview-dock--open" : ""}`}
    >
        ...
        >
          ...
          <div className="oc2-tools-preview-dock__side oc2-tools-preview-dock__side--right">
            {rightTools.length > 0 ? (
              ...
              <div className="oc2-empty-strip">
                Add more tools to fill this side.
              </div>
            )}
          </div>
        </div>
...
function QuickSettings({ isAdmin }) {
  const settings = [
    {
      title: "Editor",
      description: "Open the live Better overlay canvas.",
      to: "/editor",
      icon: Palette,
    },
    {
      title: "Integrations",
      description: "Connect chat, points and music.",
      to: "/overlay-center/integrations",
      icon: Link2,
    },
    {
  ...
  if (isAdmin) {
    settings.push({
      title: "Approvals",
      description: "Review slot submissions.",
...
function ToolWorkspace({
  ...
  const yourTools = tools.filter(
    (tool) => tool.widget && tool.widget.is_visible !== false,
  );
  const addMoreTools = tools.filter(
    (tool) => !tool.widget || tool.widget.is_visible === false,
  );
...
function summarizeBetsBracketOptions(options = []) {
  return normalizeBetsBracketOptions(options)
...
function BetsBracketShortcutTiles({ widget, saveWidget }) {
  ...
  const history = Array.isArray(config.bracketHistory)
    ? config.bracketHistory
    : [];
  const canLoad = (config.gameStatus || "idle") === "idle";

  if (
    widget?.widget_type !== "bets" ||
    (usage.length === 0 && history.length === 0)
  )
    return null;
        ...
        <div className="oc2-bets-shortcut-actions">
          <button
            ...
            title={
              canLoad
                ? "Load bracket setup"
                : "End the current round before loading a setup"
            }
          >
            Load
          </button>
      ...
      {usage.length > 0 && (
        <div className="oc2-bets-shortcut-section">
          ...
          <div className="oc2-bets-shortcut-grid">
            {usage
              .slice(0, 4)
              .map((entry, index) => renderTile(entry, index, "usage"))}
          </div>
        </div>
      )}
      ...
      {history.length > 0 && (
        <div className="oc2-bets-shortcut-section">
          ...
          <div className="oc2-bets-shortcut-grid">
            {history
              .slice(0, 4)
              .map((entry, index) => renderTile(entry, index, "history"))}
          </div>
        </div>
      )}
    </section>
...
function WidgetDetail({
  ...
  const handleAdd = async () => {
    ...
    trackEvent(ANALYTICS_EVENTS.OVERLAY_TOOL_ENABLED, {
  ...
  const handleToggle = async () => {
    ...
    trackEvent(
      widget.is_visible === false
        ? ANALYTICS_EVENTS.OVERLAY_TOOL_ENABLED
        : ANALYTICS_EVENTS.OVERLAY_TOOL_DISABLED,
      { widget_type: widgetType },
    );
  };
  ...
  const showBonusHuntHeaderCurrency =
    widgetType === "bonus_hunt" && Boolean(widget);
    ...
    return (
      <div className={`oc2-widget-status-summary ${className}`}>
        ...
        <dl className="oc2-widget-status-summary__meta">
          <div>
            <dt>Visible</dt>
            <dd>{widget.is_visible === false ? "No" : "Yes"}</dd>
          </div>
          <div>
            <dt>Layer</dt>
            <dd>{widget.z_index || 1}</dd>
          </div>
          <div>
            <dt>Size</dt>
            <dd>
  ...
  return (
    <section
      className={`oc2-detail ${detailModifier}${showHeaderTools ? " oc2-detail--has-header-tools" : ""}`}
      data-tour="widget-detail-page"
    >
            ...
            {showBonusHuntHeaderCurrency && (
              <label className="oc2-header-currency-field">
                ...
                <select
                  ...
                  onChange={handleBonusHuntCurrencyChange}
                >
      ...
      {!widget && (
        <div className="oc2-empty-state">
          ...
          <p>
            The widget will be created with safe defaults and can be configured
            here.
          </p>
          <button
            type="button"
...
function SetupWizard({
  ...
  const persist = async (next, stepToComplete = step) => {
    const completedSteps = Array.from(
      new Set([...(next.completedSteps || []), stepToComplete]),
    ).sort((a, b) => a - b);
    const updated = {
      ...next,
      completedSteps,
    ...
    setDraft(updated);
    trackEvent(ANALYTICS_EVENTS.OVERLAY_SETUP_STEP_COMPLETED, {
      step: stepToComplete + 1,
    });
    return updated;
  };
  ...
  const nextStep = async () => {
    ...
    try {
      let next = { ...draft };
      if (step === 0) {
        const [width, height] = String(next.details?.resolution || "1920x1080")
          .split("x")
          .map(Number);
        await saveTheme({
          canvas_width: width || 1920,
          canvas_height: height || 1080,
        });
      }
      if (step === 1) {
      ...
      if (step === 5) {
        if (!serviceReadiness.canContinue) {
          setActionError(
            "Finish the required service checks before continuing. Optional integrations and selected fallbacks will not block setup.",
          );
          return;
        }
      ...
      setActionError(
        error?.message || "Could not save this setup step. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const saveServiceProgress = useCallback(async () => {
    clearTimeout(serviceAutosaveRef.current);
    lastServiceAutosaveSignatureRef.current = JSON.stringify({
      details: draft.details || {},
      selectedTools: draft.selectedTools || [],
      currentStep: draft.currentStep,
    });
    ...
    <section className="oc2-setup">
      ...
      {step === 0 && (
        <div className="oc2-form-grid">
          <Field label="Overlay name">
            <input
              value={draft.details?.overlayName || ""}
              onChange={(event) =>
                patchDetails({ overlayName: event.target.value })
              }
            />
          </Field>
          <Field label="Platform">
            <select
              value={draft.details?.platform || "twitch"}
              onChange={(event) =>
                patchDetails({ platform: event.target.value })
              }
            >
          ...
          <Field label="Resolution">
            <select
              value={draft.details?.resolution || "1920x1080"}
              onChange={(event) =>
                patchDetails({ resolution: event.target.value })
              }
            >
          ...
          <Field label="Display language">
            <input
              value={draft.details?.language || "English"}
              onChange={(event) =>
                patchDetails({ language: event.target.value })
              }
            />
          </Field>
          <Field label="Timezone">
            <input
              value={draft.details?.timezone || "UTC"}
              onChange={(event) =>
                patchDetails({ timezone: event.target.value })
              }
            />
          </Field>
        </div>
          ...
          {[
            {
              id: "clean",
              name: "Clean default",
              desc: "A calm, readable broadcast base.",
            },
            {
              id: "classic",
              name: "Classic theme",
              desc: "Uses the existing classic overlay styling.",
            },
            {
              id: "metallic",
              name: "Metallic theme",
              desc: "Darker, premium and dimensional.",
            },
          ].map((style) => (
            <button
              key={style.id}
      ...
      {step === 2 && (
        <div className="oc2-form-grid">
          <Field label="Display name">
            <input
              value={draft.details?.displayName || ""}
              onChange={(event) =>
                patchDetails({ displayName: event.target.value })
              }
            />
          </Field>
          <Field label="Logo or avatar URL">
            <input
              value={draft.details?.logoUrl || ""}
              onChange={(event) =>
                patchDetails({ logoUrl: event.target.value })
              }
            />
          </Field>
          <Field label="Primary colour">
            <input
              type="color"
              value={draft.details?.primaryColor || "#14b8a6"}
              onChange={(event) =>
                patchDetails({ primaryColor: event.target.value })
              }
            />
          </Field>
          <Field label="Secondary colour">
            <input
              type="color"
              value={draft.details?.secondaryColor || "#0f172a"}
              onChange={(event) =>
                patchDetails({ secondaryColor: event.target.value })
              }
            />
          </Field>
          <Field label="Accent colour">
            <input
              type="color"
              value={draft.details?.accentColor || "#f59e0b"}
              onChange={(event) =>
                patchDetails({ accentColor: event.target.value })
              }
            />
          </Field>
        </div>
        ...
        <div className="oc2-select-tools">
          {PRIMARY_TOOLS.filter((type) => getWidgetDef(type)).map((type) => (
            <label key={type} className="oc2-tool-check">
                ...
                onChange={(event) => {
                  setDraft((prev) => ({
                    ...
                    selectedTools: event.target.checked
                      ? Array.from(
                          new Set([...(prev.selectedTools || []), type]),
                        )
              ...
              <span>
                <strong>
                  {FEATURE_COPY[type]?.title || getWidgetDef(type)?.label}
                </strong>
                <small>
                  {FEATURE_COPY[type]?.description ||
                    getWidgetDef(type)?.description}
                </small>
              </span>
            </label>
          ))}
      ...
      {step === 4 && (
        <div className="oc2-panel">
          ...
          <p>
            We will create any missing selected tools with safe defaults.
            Detailed settings remain available later from each tool page.
          </p>
      ...
      {step === 5 && (
        <ConnectServicesStep
          ...
          onDetailsChange={(nextDetails) =>
            setDraft((prev) => ({ ...prev, details: nextDetails }))
          }
      ...
      {step === 6 && (
        <div className="oc2-panel">
          ...
          <p>
            Selected tools: {selectedTools.length}. Browser-source URL:{" "}
            {instance?.overlay_token ? "active" : "missing"}.
          </p>
    ...
    <section className="oc2-preview-page" data-tour="preview-page">
      <div className="oc2-detail-header">
        <div>
          ...
          <p>
            Inline preview uses the same tokenized browser-source URL that OBS
            uses.
          </p>
        </div>
        <div className="oc2-detail-actions">
          ...
          <button type="button" className="oc2-btn" onClick={onFocus}>
            Focus preview
          </button>
          <button type="button" className="oc2-btn" onClick={onClose}>
            Close preview
          </button>
          <button
            type="button"
            ...
            data-tour="obs-url"
          >
            <Copy size={16} /> {copyMsg || "Copy OBS URL"}
          </button>
        </div>
      </div>
      ...
      {overlayUrl ? (
        <div className="oc2-inline-preview">
          <iframe
            ...
            src={overlayUrl}
            onLoad={() =>
              trackEvent(ANALYTICS_EVENTS.OVERLAY_PREVIEW_CONNECTED, {
...
function IntegrationGrid({ selectedTools }) {
  return (
    <div
      className="oc2-integration-grid oc2-integration-grid--modern"
      data-tour="integrations-overview"
    >
      {INTEGRATIONS.map((item) => {
        const related = selectedTools.filter((type) =>
          item.requiredFor.includes(type),
        );
        return (
          <article
            key={item.id}
            className={`oc2-integration-card oc2-integration-card--${item.id}${related.length ? " oc2-integration-card--relevant" : ""}`}
          >
            <div className="oc2-integration-card__top">
              <span
                className="oc2-integration-card__indicator"
                aria-hidden="true"
              />
              <strong>{item.name}</strong>
              <span
                className={`oc2-pill ${related.length ? "oc2-pill--gold" : ""}`}
              >
                {related.length ? "Relevant" : "Optional"}
              </span>
            </div>
            <p>{item.detail}</p>
            <small>
              {related.length > 0
                ? `Used by ${related.map((type) => FEATURE_COPY[type]?.title || type).join(", ")}`
                : "Available when a tool needs it"}
            </small>
          </article>
        );
      })}
...
export default function OverlayControlCenter() {
  ...
  const widgetTypeFromRoute = useMemo(
    () => fromSlug(location.pathname.split("/").pop()),
    [location.pathname],
  );
  const integrations = useMemo(() => {
    const spotifyWidget = widgets.find(
      (widget) => widget.widget_type === "spotify_now_playing",
    );
    const navbarWidget = widgets.find(
      (widget) => widget.widget_type === "navbar",
    );
    return {
      twitchChannel,
      streamelementsConnected: !!(
        seAccount?.se_channel_id && seAccount?.se_jwt_token
      ),
      spotifyConnected: !!(
  ...
  useEffect(() => {
    ...
    channel.onmessage = (event) => {
      ...
      if (event.data?.type === "overlay-preview-closed")
        setPreviewStatus("closed");
      if (event.data?.type === "overlay-preview-disconnected")
  ...
  const goToTutorialPage = useCallback(
    (page) => {
      const routeMap = {
        home: "/overlay-center",
  ...
  useEffect(() => {
    const isWidgetDetailPath = location.pathname.startsWith(
      "/overlay-center/widgets/",
    );
    const isKnownPanelPath = Boolean(PANEL_ROUTES[location.pathname]);
    if (
      location.pathname.startsWith("/overlay-center/") &&
      !isWidgetDetailPath &&
      !isKnownPanelPath
    ) {
      navigate("/overlay-center", { replace: true });
  ...
  const openPreview = useCallback(() => {
    ...
    trackEvent(ANALYTICS_EVENTS.OVERLAY_PREVIEW_POPPED_OUT, {
      overlay_id: instance?.id,
    });
  ...
  const syncThemeToWidgets = useCallback(
    async (themeId, metalPresetId) => {
      const t = themeMap[themeId];
      if (!t || !widgets?.length) return;
      let colors = { ...t.colors };
      if (themeId === "metallic" && metalPresetId) {
        const preset = (await import("../../data/appThemes")).metallicPresets[
          metalPresetId
        ];
        if (preset)
          colors = { ...colors, primary: preset.hex, accent: preset.hex };
      }
      await saveTheme({
      ...
      if (navWidget)
        await saveWidget({
          ...
          config: {
            ...navWidget.config,
            ...themeColors,
            displayStyle: themeId === "metallic" ? "metallic" : "glass",
          },
        });
  ...
  const handleRemoveTool = async (widgetId) => {
    const ok = window.confirm(
      "Remove this tool from the overlay? This cannot be undone.",
    );
    if (!ok) return;
        ...
        <section className="oc2-empty-state">
          ...
          <p>
            {overlayError.message ||
              "The overlay data request did not complete. Please try again."}
          </p>
          <button
            type="button"
            className="oc2-btn oc2-btn--primary"
  ...
  if (location.pathname === "/overlay-center/appearance") {
    ...
    <div className="oc-page oc2-page">
      ...
      >
        ...
        {currentPanel === "integrations" && (
          <section
            className="oc2-integrations-page"
          ...
          >
            <div className="oc2-section-heading oc2-integrations-heading">
              ...
              <p>
                Set up the accounts your overlay tools use. Keep your profile,
                platform channels, music and StreamElements credentials in one
                place.
              </p>
            </div>
            <IntegrationGrid
        ...
        {currentPanel === "slots" && !(isPremium || isAdmin) && (
          <section className="oc2-empty-state">
            ...
            <Link className="oc2-btn oc2-btn--primary" to="/premium">
              View Premium
            </Link>
          </section>
        )}
        {currentPanel === "approvals" && isAdmin && (
          <div data-tour="approvals-page">
            <SlotApprovals />
          </div>
        )}
        {currentPanel === "approvals" && !isAdmin && (
          <section className="oc2-empty-state">
            ...
            <Link className="oc2-btn" to="/overlay-center">
              Back to Overlay Center
            </Link>
          </section>
        )}

...
const S = {
  ...
  identity: {
    ...
    padding: "16px",
    background:
      "linear-gradient(135deg, rgba(20,184,166,0.1), rgba(124,58,237,0.08) 42%, rgba(13,18,28,0.88))",
    border: "1px solid rgba(94,234,212,0.2)",
    borderRadius: 18,
    boxShadow:
      "inset 0 1px 0 rgba(255,255,255,0.06), 0 18px 48px rgba(0,0,0,0.34)",
  },
  avatar: {
    width: 58,
    height: 58,
  ...
  card: {
    background:
      "linear-gradient(150deg, rgba(17,24,39,0.82), rgba(8,13,22,0.92))",
    ...
    gap: 12,
    boxShadow:
      "inset 0 1px 0 rgba(255,255,255,0.05), 0 14px 38px rgba(0,0,0,0.28)",
  },
  cardTitle: {
    fontSize: "0.92rem",
    fontWeight: 850,
  ...
  dot: (on) => ({
    ...
    height: 8,
    borderRadius: "50%",
    background: on ? "#5eead4" : "#64748b",
    flexShrink: 0,
    boxShadow: on ? "0 0 18px rgba(94,234,212,0.38)" : "none",
  }),
  /* buttons */
  btn: {
    minHeight: 38,
  ...
  syncBar: {
    ...
    padding: "14px 16px",
    background:
      "linear-gradient(135deg, rgba(37,99,235,0.12), rgba(17,24,39,0.86))",
    border: "1px solid rgba(96,165,250,0.22)",
    borderRadius: 18,
    boxShadow:
      "inset 0 1px 0 rgba(255,255,255,0.05), 0 14px 38px rgba(0,0,0,0.26)",
  },
  syncResult: {
    fontSize: "0.8rem",
    padding: "9px 14px",
  ...
  return (
    <div style={S.card}>
      ...
      {!hasAccess ? (
        ...
        >
          API access not enabled for your account. Ask the site admin to grant
          you access from the Admin Panel → API Keys tab.
        </p>
      ) : !apiKey ? (
        <div>
          <p
          ...
          <button
            style={{
              ...S.btn,
              background:
                "linear-gradient(135deg, #94a3b8 0%, #7c8797 48%, #64748b 100%)",
              color: "#f4f7fb",
              fontWeight: 800,
              width: "100%",
            }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          >
            <span>
              {apiKey.api_key.slice(0, 12)}...{apiKey.api_key.slice(-8)}
            </span>
            <button
              onClick={copyKey}
              style={{
          ...
          <div
            style={{
              ...
              gap: 6,
              fontSize: "0.72rem",
              color: apiKey.is_active ? "#d0dbe6" : "#fca5a5",
            }}
          >
            <div
              style={{
                ...
                height: 6,
                borderRadius: "50%",
                background: apiKey.is_active ? "#d0dbe6" : "#ef4444",
              }}
            />
            {apiKey.is_active ? "Active" : "Disabled"}
            {apiKey.last_used_at && (
              <span style={{ color: "#6d7a8d" }}>
                {" "}
...
export default function ProfileSection({ widgets, saveWidget }) {
  ...
  useEffect(() => {
    ...
    const nb =
      (widgets || []).find((w) => w.widget_type === "navbar")?.config || {};
    const chat =
      (widgets || []).find((w) => w.widget_type === "chat")?.config || {};
    const ga =
      (widgets || []).find((w) => w.widget_type === "giveaway")?.config || {};
    ...
    (async () => {
      ...
      setProfile((prev) => ({
        ...
        twitchUsername:
          ...
          "",
        kickChannel:
          chat.kickChannelId || ga.kickChannelId || prev.kickChannel || "",
  ...
  useEffect(() => {
      ...
      ws.onmessage = async (event) => {
        for (const line of event.data.split("\r\n")) {
          ...
          const songMatch = line.match(
            /:(\w+)!\w+@[\w.]+\.tmi\.twitch\.tv PRIVMSG #\w+ :!song (.+)/i,
          );
          if (songMatch) {
            ...
            if (songName) {
              try {
                await fetch(
                  `${window.location.origin}/api/chat-commands?cmd=song&user_id=${encodeURIComponent(user.id)}&song=${encodeURIComponent(songName)}`,
                );
              } catch (err) {
                console.error("[SongRequest] IRC error", err);
              }
            }
          }
    ...
    return () => {
      ...
      if (songWsRef.current) {
        songWsRef.current.close();
        songWsRef.current = null;
      }
    };
  ...
  const connectedPlatforms = useMemo(() => {
    ...
    if (profile.kickChannel)
      list.push({ name: "Kick", user: profile.kickChannel, color: "#b8c8d8" });
    ...
    if (widgets && saveWidget) {
      ...
      for (const w of widgets) {
        if (w.widget_type === "navbar") {
          await saveWidget({
            ...w,
            config: { ...w.config, ...clearPayload, musicSource: "spotify" },
          });
        }
      }
    }
  ...
  const syncToWidgets = useCallback(async () => {
    if (!widgets || widgets.length === 0) {
      setSyncMsg("⚠️ No widgets to sync to");
      return;
    }
    setSaving(true);
    setSyncMsg("");
    ...
    try {
      ...
      for (const widget of widgets) {
        ...
        if (
          widget.widget_type === "navbar" &&
          profile.spotify_access_token &&
          updates.spotify_access_token
        ) {
        ...
        if (changed) {
          savePromises.push(
            saveWidget({ ...widget, config: { ...widget.config, ...updates } }),
          );
          synced++;
        }
      }
  ...
  const handleSyncAll = async () => {
    ...
    if (profile.twitchUsername) {
      ...
      localStorage.setItem(
        "streamerName",
        profile.streamerName || profile.twitchUsername,
      );
      window.dispatchEvent(
        new CustomEvent("streamerNameChanged", {
          detail: { name: profile.twitchUsername },
        }),
      );
    }
  };
  ...
  return (
    <div className="oc-profile-page" style={S.page}>
      ...
      >
        ...
        <span
          className="oc-profile-provider-badge"
          style={{
            fontSize: "0.7rem",
            color: "#aab6c8",
            whiteSpace: "nowrap",
      ...
      <div className="oc-profile-grid" style={S.grid}>
        ...
        <div
          className="oc-profile-column"
          style={{ display: "flex", flexDirection: "column", gap: 16 }}
        >
          <div
            className="oc-profile-card oc-profile-card--platforms"
            style={{ ...S.card, gap: 10 }}
            data-tour="profile-platforms"
          >
            <h3 style={S.cardTitle}>🔗 Platforms</h3>

            <div
              className="oc-profile-platform-row"
              style={{ display: "flex", alignItems: "center", gap: 8 }}
            >
              <div style={S.dot(!!profile.twitchUsername)} />
              <label style={{ ...S.label, margin: 0, minWidth: 60 }}>
                Twitch
              </label>
              <input
            ...
            <div
              className="oc-profile-platform-row oc-profile-platform-row--youtube"
              style={{ display: "flex", alignItems: "center", gap: 8 }}
            >
              ...
              <input
                ...
                placeholder="Video / Live ID"
              />
              <input
                style={{ ...S.input, flex: 1, margin: 0 }}
                type="password"
                value={profile.youtubeApiKey}
            ...
            <button
              style={{
                ...S.btn,
                background: platformSaved
                  ? "rgba(208,219,230,0.16)"
                  : "rgba(17,24,39,0.58)",
                color: platformSaved ? "#d0dbe6" : "#aab6c8",
                border: "1px solid rgba(148,163,184,0.34)",
                width: "100%",
                marginTop: 4,
              }}
          ...
          <div
            className="oc-profile-sync-card"
            style={{ ...S.syncBar, borderRadius: 18 }}
            data-tour="profile-sync"
          >
            <div
              style={{
                flex: 1,
            ...
            >
              <span
                style={{ fontSize: "0.84rem", fontWeight: 700, color: "#fff" }}
              >
                📡 Widget Sync
              </span>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {(widgets || [])
                  .filter((w) => SYNC_MAP[w.widget_type])
                  .map((w) => (
                    <span
                      key={w.id}
                      style={{
                ...
                {(widgets || []).filter((w) => SYNC_MAP[w.widget_type])
                  .length === 0 && (
                  <span style={{ fontSize: "0.72rem", color: "#aab6c8" }}>
                    Add widgets to enable sync
                  </span>
                )}
              </div>
            </div>
            ...
            <div
              style={{
                ...
                background: syncMsg.startsWith("✅")
                  ? "rgba(208,219,230,0.08)"
                  : syncMsg.startsWith("❌")
                    ? "rgba(148,163,184,0.1)"
                    : "rgba(200,208,216,0.1)",
          ...
          <div
            className="oc-profile-card oc-profile-card--obs"
            data-tour="profile-obs-guide"
            style={{ ...S.card }}
          >
            ...
            >
              Add your overlay as a Browser Source in OBS from{" "}
              <strong style={{ color: "#d0dbe6" }}>/editor</strong>. Publish the
              Better layout, copy the full overlay or individual widget URL
              there, and use{" "}
              <strong style={{ color: "#d0dbe6" }}>1920×1080</strong> for the
              full overlay source.
            </p>
            <p
              style={{
            ...
            >
              If the overlay looks blurry, right-click the source in OBS →{" "}
              <strong style={{ color: "#d0dbe6" }}>
                Transform → Edit Transform
              </strong>{" "}
              and make sure the size matches your canvas resolution exactly.
            </p>
          </div>
        </div>
        ...
        >
          {/* Spotify card */}
          <div
            className="oc-profile-card oc-profile-card--spotify"
            style={S.card}
            data-tour="profile-spotify"
          >
            ...
            <div style={S.platRow}>
              ...
              <span
                style={{
                  fontSize: "0.82rem",
                  color: profile.spotify_access_token ? "#d0dbe6" : "#aab6c8",
                  fontWeight: 600,
                  flex: 1,
                }}
              >
            ...
            {spotifyError && (
              <p style={{ fontSize: "0.74rem", color: "#f87171", margin: 0 }}>
                {spotifyError}
              </p>
            )}
            <p
            ...
            {user && profile.twitchUsername && (
              <div
                style={{
                  marginTop: 8,
                  padding: "10px 12px",
                  background: "rgba(208,219,230,0.08)",
                  borderRadius: 16,
                  border: "1px solid rgba(208,219,230,0.22)",
                }}
              >
                <p
                  style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  >
                    {songIrcStatus === "live"
                      ? `Listening to #${profile.twitchUsername}`
                      : songIrcStatus === "connecting"
                        ? "Connecting…"
                        : "Not connected"}
                  </span>
                ...
                {songIrcStatus === "live" && (
                  ...
                  >
                    ...
                    in chat to queue songs on Spotify
                  </p>
                )}
                {songIrcStatus === "off" && profile.twitchUsername && (
                  ...
                  >
                    Will auto-connect when Spotify is linked and Twitch username
                    is set
                  </p>
                )}
              </div>
          ...
          {/* StreamElements card */}
          <div
            className="oc-profile-card oc-profile-card--streamelements"
            style={S.card}
            data-tour="profile-streamelements"
          >
            ...
            <div>
              ...
              <input
                style={S.input}
                value={profile.seChannelId}
                onChange={(e) => set("seChannelId", e.target.value)}
            ...
            <div>
              ...
              <input
                style={S.input}
                type="password"
                value={profile.seJwtToken}
                onChange={(e) => set("seJwtToken", e.target.value)}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                ...
                disabled={
                  seSaving || !profile.seChannelId || !profile.seJwtToken
                }
                onClick={async () => {
                  setSeSaving(true);
                  setSeTestMsg("");
            ...
            {seTestMsg && (
              <p
                style={{
                  fontSize: "0.74rem",
                  color: seTestMsg.startsWith("✅")
                    ? "#d0dbe6"
                    : seTestMsg.startsWith("❌")
                      ? "#fca5a5"
                      : "#b8c8d8",
                  margin: 0,
                  fontWeight: 600,
            ...
            >
              ...
              → Account → Channels.
              <br />
              Each user must enter their own credentials. Saved per-account.
            </p>
          </div>

          {/* Preferences card */}
          <div
            className="oc-profile-card oc-profile-card--preferences"
            style={S.card}

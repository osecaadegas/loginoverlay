...
- Original (protected Bonus Hunt baseline; emits no generic visual overrides)
- Matte
- Metallic
- Gradient
- Glass
...
`bonus_hunt` uses Original as its canonical default. Missing settings and Reset do not generate generic V2 surfaces. Non-Original material presets are intentionally additive and map mainly into existing Bonus Hunt color/text variables:

- `headerColor`, `headerAccent`, `listCardColor`, `summaryColor`, `totalPayColor`
- `textColor`, `mutedTextColor`, `statValueColor`, safe value/state colours
- state styles for positive/negative values and footer success/error

Structural Bonus Hunt values remain widget-owned and are not generated by presets:

- Carousel dimensions and animation distances.
- Slot image height.
- Slot row padding and height.
- Header/footer padding.
- Root frame and core internal gaps.
...
- This pilot document covers only `bh_stats` and `bonus_hunt`; later migrated widgets are documented in their own implementation notes.

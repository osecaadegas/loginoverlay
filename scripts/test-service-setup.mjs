...
const clientSource = readFileSync(
  new URL("../src/services/serviceReadinessService.js", import.meta.url),
  "utf8",
);

test("Twitch login is prefilled and normalized from integrations", () => {
  const details = normalizeSetupDetails(
    {},
    { twitchChannel: "#OSeCaaDegas", twitchDisplayName: "Os e Caa Degas" },
  );
...
test("Guided setup does not write or render secrets in client fields", () => {
  ...
  assert.doesNotMatch(
    componentSource,
    /se_jwt_token|access_token|refresh_token|client_secret/i,
  );
});
...
test("Duplicate chat commands are blocking validation errors", () => {
  const validation = validateCommandConfiguration({
    slotRequestCommand: "!sr",
    betCommand: "/sr",
    giveawayKeyword: "join",
  });
...
test("Invalid command characters are rejected", () => {
  const validation = validateCommandConfiguration({
    slotRequestCommand: "slot request",
    betCommand: "bet",
    giveawayKeyword: "join",
  });
...
test("Negative point costs are validation errors", () => {
  const validation = validatePointSettings({
    requestCost: -1,
    giveawayEntryCost: -2,
  });
  assert.ok(
    validation.errors.some((error) => error.includes("Slot request cost")),
  );
  assert.ok(
    validation.errors.some((error) => error.includes("Giveaway entry cost")),
  );
});
...
test("Currency is stored as an ISO code and rendered from metadata", () => {
  assert.equal(
    normalizeSetupDetails({ currency: "\u20ac" }).currencyCode,
    "EUR",
  );
  assert.equal(
    normalizeSetupDetails({ currencyCode: "usd" }).currencyCode,
    "USD",
  );
...
test("Safe destinations reject scripts and token-bearing URLs but keep internal routes", () => {
  assert.equal(
    safeExternalDestination("javascript:alert(1)", "/fallback"),
    "/fallback",
  );
  assert.equal(
    safeExternalDestination(
  ...
  assert.equal(
    safeExternalDestination("/overlay-center/setup#slot-data", "/fallback"),
    "/overlay-center/setup#slot-data",
  );
});
...
test("Centralized service links do not contain secret query parameters", () => {
  ...
  assert.ok(
    urls.every(
      (url) =>
        !/[?&#](access_token|refresh_token|id_token|client_secret|jwt)=/i.test(
          url,
        ),
    ),
  );
...
test("Music modes normalize Spotify, manual and disabled states", () => {
  assert.equal(
    normalizeSetupDetails({ spotifyMode: "spotify" }).musicMode,
    "spotify",
  );
  const manual = normalizeSetupDetails({
    musicMode: "manual",
    manualTrack: " Song ",
    manualArtist: " Artist ",
  });
  ...
  assert.equal(
    normalizeSetupDetails({ musicMode: "disabled" }).musicMode,
    "disabled",
  );
});
...
test("Manual slot source enables fallback and trims sample search text", () => {
  const details = normalizeSetupDetails({
    slotSource: "manual",
    manualSlotFallback: false,
    sampleSlotName: " Gates of Olympus ",
  });
...
test("Final service review displays commands, currency and fallback choices", () => {
  const review = buildFinalServiceReview({
    commandPrefix: "?",
    slotRequestCommand: "sr",
    betCommand: "bet",
    currencyCode: "GBP",
    slotSource: "manual",
    manualSlotFallback: true,
  });

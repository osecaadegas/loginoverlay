-- Consolidated migration: 009_analytics.sql
-- Generated from active source migrations retained after cleanup

-- ============================================================================
-- Source: analytics_system.sql
-- ============================================================================
-- ============================================================
-- ANALYTICS SYSTEM  Full Schema
...
AS $$
BEGIN
  IF table_name <> 'analytics_visitors' THEN
    RAISE EXCEPTION 'increment_field only supports analytics_visitors';
  END IF;

  IF field_name NOT IN ('total_sessions', 'total_events') THEN
    RAISE EXCEPTION 'increment_field only supports analytics visitor counters';
  END IF;

  EXECUTE format(
    'UPDATE analytics_visitors SET %1$I = COALESCE(%1$I, 0) + $1, last_seen_at = now() WHERE id = $2',
    field_name
  )
  USING amount, row_id;
END;
...
GRANT EXECUTE ON FUNCTION increment_field(TEXT, UUID, TEXT, INT) TO anon;
GRANT EXECUTE ON FUNCTION increment_field(TEXT, UUID, TEXT, INT) TO authenticated;

-- ============================================================================
-- Source: fix_analytics_views_security.sql
-- ============================================================================
-- ============================================================

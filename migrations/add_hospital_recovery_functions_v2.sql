...
BEGIN
  ...
  IF v_player.hospital_until IS NULL OR v_player.hospital_until <= NOW() THEN
    ...
    VALUES (v_player.id, v_player_id, 'exploit_attempt', 'hospital_recovery', 
      jsonb_build_object('request_id', v_request_id, 'reason', 'not_in_hospital'), 
      true, 'Tried to recover when not hospitalized');
  ...
  IF v_total_wealth < v_recovery_cost THEN
    ...
    VALUES (v_player.id, v_player_id, 'action', 'hospital_recovery_failed', jsonb_build_object(
  ...
  VALUES (v_player.id, v_player_id, 'action', 'hospital_recovery_success', 
    jsonb_build_object('cash', v_player.cash, 'bank', v_player.bank_balance, 'hp', v_player.hp),
    jsonb_build_object('cash', v_new_cash, 'bank', v_new_bank, 'hp', v_player.max_hp),
    jsonb_build_object('request_id', v_request_id, 'cost', v_recovery_cost),
    false);
...
BEGIN
  ...
  -- Log success
  INSERT INTO the_life_security_logs (player_id, user_id, event_type, action_name, old_values, new_values, metadata, is_flagged)
  VALUES (v_player.id, v_player_id, 'action', 'intense_treatment_success',
    jsonb_build_object('cash', v_player.cash, 'bank', v_player.bank_balance, 'addiction', v_player.addiction),
    jsonb_build_object('cash', v_new_cash, 'bank', v_new_bank, 'addiction', 0),
    jsonb_build_object('request_id', v_request_id, 'cost', v_treatment_cost),
    false);
...
BEGIN
  ...
  -- Log
  INSERT INTO the_life_security_logs (player_id, user_id, event_type, action_name, old_values, new_values, metadata, is_flagged)
  VALUES (v_player.id, v_player_id, 'action', 'hospital_service',
    jsonb_build_object('hp', v_player.hp, 'cash', v_player.cash),
    jsonb_build_object('hp', LEAST(v_player.max_hp, v_player.hp + v_hp_restore), 'cash', v_player.cash - v_cost),
    jsonb_build_object('request_id', v_request_id, 'service', p_service_type, 'cost', v_cost, 'hp_restored', LEAST(v_player.max_hp - v_player.hp, v_hp_restore)),
    false);
...
BEGIN
  ...
  -- Log
  INSERT INTO the_life_security_logs (player_id, user_id, event_type, action_name, old_values, new_values, metadata, is_flagged)
  VALUES (v_player.id, v_player_id, 'action', 'addiction_cured',
    jsonb_build_object('addiction', v_player.addiction, 'cash', v_player.cash),
    jsonb_build_object('addiction', 0, 'cash', v_player.cash - v_cure_cost),
    jsonb_build_object('request_id', v_request_id, 'cost', v_cure_cost),
    false);

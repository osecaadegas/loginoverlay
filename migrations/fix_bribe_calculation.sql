-- =====================================================
-- FIX: Bribe Calculation Formula Mismatch
-- =====================================================
...
BEGIN
  ...
  IF v_player.jail_until IS NULL OR v_player.jail_until <= NOW() THEN
    ...
    VALUES (v_player_id, 'bribe_attempt_not_in_jail', 
  ...
  WHERE player_id = v_player_id
    AND action_type IN ('bribe_attempt', 'bribe_success', 'bribe_failed')
  ...
  -- Log the attempt
  INSERT INTO the_life_security_logs (player_id, action_type, details, is_suspicious)
  VALUES (v_player_id, 'bribe_attempt', jsonb_build_object(
  ...
  IF v_player.cash < v_bribe_amount THEN
    INSERT INTO the_life_security_logs (player_id, action_type, details, is_suspicious)
    VALUES (v_player_id, 'bribe_failed', jsonb_build_object(
      'request_id', v_request_id,
  ...
  ELSE
    ...
    INSERT INTO the_life_security_logs (player_id, action_type, details, is_suspicious)
    VALUES (v_player_id, 'bribe_success', jsonb_build_object(
      'request_id', v_request_id,
